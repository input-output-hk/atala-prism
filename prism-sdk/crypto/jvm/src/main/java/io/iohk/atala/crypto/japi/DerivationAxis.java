...
public class DerivationAxis {
    ...
    public boolean isHardened() {
        return io.iohk.atala.crypto.DerivationAxis.isHardened$extension(i);
    }

    /**
     * Returns number corresponding to the axis (different for index), always between 0 and 2^31^ */
    public int getNumber() {
        return io.iohk.atala.crypto.DerivationAxis.number$extension(i);
    }

    public String axisToString() {
        return io.iohk.atala.crypto.DerivationAxis.axisToString$extension(i);
    }
    ...
    public static DerivationAxis normal(int num) {
        return new DerivationAxis(io.iohk.atala.crypto.DerivationAxis.normal(num));
    }

    public static DerivationAxis hardened(int num) {
        return new DerivationAxis(io.iohk.atala.crypto.DerivationAxis.hardened(num));
    }
}
