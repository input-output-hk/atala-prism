...
class PayIdAddressListFragment : DaggerFragment(), OnSelectItem<PayIdAddress> {
    @Inject
    ...
    private val listAdapter = AddressListAdapter(this)
    ...
    override fun onSelect(item: PayIdAddress) {
        if (requireContext().copyToClipBoard("", item.address)) {
