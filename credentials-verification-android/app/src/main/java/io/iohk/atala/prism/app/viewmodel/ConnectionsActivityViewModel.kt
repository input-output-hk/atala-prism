...
import io.iohk.atala.prism.app.viewmodel.dtos.ContactToDelete
...
class ConnectionsActivityViewModel @Inject constructor(val dataManager: DataManager) : NewConnectionsViewModel(dataManager) {
    ...
    private val _showContactToDeleteDialog = MutableLiveData<ContactToDelete>()
    fun getContactToDeleteLiveData(): LiveData<ContactToDelete> = _showContactToDeleteDialog
    ...
    fun getCredentialsToShareLiveData(): LiveData<AsyncTaskResult<List<CredentialsToShare?>>> {
        return _credentialsToShare
    }

    fun getContactsLiveData(): LiveData<AsyncTaskResult<List<Contact?>>> {
    ...
    fun getAllMessages(): MutableLiveData<AsyncTaskResult<List<ReceivedMessage>>>? {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                val proofRequests = messagesPaginatedResponseList.filter { receivedMessage ->
    ...
    private suspend fun saveCredentials(messagesPaginatedResponseList: List<ReceivedMessage>) {
        val credentialsList = messagesPaginatedResponseList.filter {
            val newMessage: AtalaMessage = AtalaMessage.parseFrom(it.message)
            newMessage.proofRequest.typeIdsList.isEmpty()
        }.map { receivedMessage: ReceivedMessage? ->
    ...
    fun getContactInfoToDelete(contact: Contact) {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                val credentialsList = dataManager.getCredentialsByConnectionId(contact.connectionId)
                _showContactToDeleteDialog.postValue(ContactToDelete(credentialsList, contact))
            } catch (ex: Exception) {
    ...
    fun deleteContact(contact: Contact) {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                dataManager.deleteCredentialByContactId(contact.connectionId)
                dataManager.deleteContact(contact)
                getAllMessages()
                _showContactToDeleteDialog.postValue(null)
            } catch (ex: Exception) {
