...
private[services] class ObjectManagementServiceLogs[F[_]: ServiceLogging[*[_], ObjectManagementService[F]]: MonadThrow]
  ...
  def saveObject(notification: AtalaObjectNotification): Mid[F, Either[SaveObjectError, Boolean]] =
    in =>
  ...
  def getOperationInfo(atalaOperationId: AtalaOperationId): Mid[F, Option[AtalaOperationInfo]] =
    in =>
      info"getting operation info by AtalaOperationId ${atalaOperationId.hexValue}" *> in
        .flatTap {
          _.fold(error"getting operation info by AtalaOperationId - finished with result: None")(info =>
            info"""getting operation info by AtalaOperationId - finished with result:
  ...
  private def logScheduleOperationResult(result: Either[errors.NodeError, AtalaOperationId]): F[Unit] =
    result.fold(
