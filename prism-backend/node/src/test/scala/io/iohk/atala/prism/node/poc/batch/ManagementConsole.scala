...
case class ManagementConsole(node: node_api.NodeServiceGrpc.NodeServiceBlockingStub) {
  ...
  def issueCredentialBatch(issueCredentialBatchOperation: SignedAtalaOperation): String = {
    // First some storage stuff to mark a credential as stored
    // It then posts the operation to the node
    node.issueCredentialBatch(IssueCredentialBatchRequest(Some(issueCredentialBatchOperation))).batchId
  }

  def revokeCredentialBatch(revokeCredentialBatchOperation: SignedAtalaOperation): Unit = {
    // First storage stuff
  ...
  def revokeSpecificCredentials(revokeCredentialBatchOperation: SignedAtalaOperation): Unit = {
    // First storage stuff
    // then, posting things on the blockchain through the node
    node.revokeCredentials(RevokeCredentialsRequest(Some(revokeCredentialBatchOperation)))
    ()
