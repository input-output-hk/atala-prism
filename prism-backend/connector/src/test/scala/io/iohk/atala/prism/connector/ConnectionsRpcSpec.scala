...
class ConnectionsRpcSpec extends ConnectorRpcSpecBase with MockitoSugar {
  ...
  "RevokeConnection" should {
    "work" in {
      ...
      // create connection
      val connectionId =
        usingApiAs(Random.nextBytes(80).toVector, keys, addTokenRequest) {
          blockingStub =>
      ...
      usingApiAs(Random.nextBytes(80).toVector, keys, revokeRequest) {
        blockingStub =>
    ...
    "work with unpublished did auth" in {
      ...
      // create connection
      val connectionId = usingApiAs(uhpublishedDidAddTokenRequest) {
        blockingStub =>
          val response = blockingStub.addConnectionFromToken(addTokenRequest)
          val connectionId =
            ConnectionId.unsafeFrom(response.connection.value.connectionId)
  ...
  "getConnectionCommunicationKeys" should {
    ...
    "return connection keys for a participant with key known to connector" in {
      ...
      val request =
        connector_api.GetConnectionCommunicationKeysRequest(connectionId =
  ...
  private def testReturningNonRevokedKeysForADIDOwningParticipant(
    ...
    val response = node_api.GetDidDocumentResponse(
      Some(
        node_models.DIDData(
          ...
          publicKeys =
            issuerCommKeys.map { case (keyId, key, revokedTimestamp, usage) =>
    ...
    usingApiAs(rpcRequest) { blockingStub =>
      ...
      val expectedKeys =
        issuerCommKeys.filter(k => expectedKeyNames.contains(k._1)).map {
          case (keyId, key, _, _) =>
