...
class RegistrationRpcSpec extends ConnectorRpcSpecBase {
  ...
  "registerDID" should {
    "work" in {
      usingApiAs.unlogged { blockingStub =>
        ...
        checkParticipantProperlyStored(response, expectedDID, name, logo, transactionId.some, Ledger.InMemory.some)
      }
    }
    ...
    "work then user uses existing did" in {
      usingApiAs.unlogged { blockingStub =>
        ...
        val response = blockingStub.registerDID(request)
        checkParticipantProperlyStored(response, did, name, logo, none, none)
      }
    }
    ...
    "fail when participant with passed did already exists" in {
      usingApiAs.unlogged { blockingStub =>
        ...
        val existingParticipantInfo =
          ParticipantInfo(participantId, participantRole, didKeyPair.publicKey.some, name, did.some, None, none, none)
  ...
  private def checkParticipantProperlyStored(
      ...
      expectedName: String,
      logo: Array[Byte],
      expectedTransactionId: Option[TransactionId],
      expectedLedger: Option[Ledger]
  ): Assertion = {
    // returns the did
    ...
    participant.transactionId mustBe expectedTransactionId
    participant.ledger mustBe expectedLedger
  }
