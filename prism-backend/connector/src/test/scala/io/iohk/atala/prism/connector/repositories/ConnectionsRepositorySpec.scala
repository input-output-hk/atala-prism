...
class ConnectionsRepositorySpec extends ConnectorRepositorySpecBase {
  lazy val connectionsRepository =
    ConnectionsRepository.unsafe(dbLiftedToTraceIdIO, connectorRepoSpecLogs)
  ...
  private def checkConnection(
    ...
    sql"""SELECT COUNT(1) FROM connection_tokens WHERE token=$token AND used_at IS NOT NULL"""
      .runUnique[Int]() mustBe 1
    sql"""SELECT COUNT(1) FROM connection_tokens WHERE token=$token AND used_at IS NULL"""
      .runUnique[Int]() mustBe 0
  }
    ...
    "correctly generate tokens" in {
      ...
      val result =
        connectionsRepository
          .insertTokens(issuerId, List(token1, token2))
          .run(TraceId.generateYOLO)
          .unsafeRunSync()
      result.nonEmpty mustBe true

      sql"""SELECT COUNT(1) FROM connection_tokens WHERE token=$token1 OR token=$token2"""
        .runUnique[Int]() mustBe 2
    }
  }
  ...
  "getTokenInfo" should {
    "return token info" in {
      ...
      sql"""INSERT INTO connection_tokens(token, initiator) VALUES ($token, $issuerId)"""
        .runUpdate()
  ...
  "addConnectionFromToken" should {
    "add connection from existing token using public key auth" in {
      ...
      val maybeAcceptorId = ConnectionsDAO
        .getRawConnection(connectionId)
        .unsafeRun()
        .map(_.acceptor)
      maybeAcceptorId.isDefined mustBe true
      val acceptor =
        ParticipantsDAO.findByPublicKey(publicKey).value.unsafeRun()
    ...
    "add connection from existing token using unpublished did auth" in {
      ...
      val result =
        connectionsRepository
          .addConnectionFromToken(token, did.asLeft)
          .run(TraceId.generateYOLO)
          .unsafeRunSync()
      val connectionId = result.toOption.value.id
  ...
  "revokeConnection" should {
    ...
    "work when the initiator revokes the connection" in {
      ...
      val result =
        connectionsRepository
          .revokeConnection(initiator, connectionId)
          .run(TraceId.generateYOLO)
          .unsafeRunSync()
    ...
    "work when the acceptor revokes the connection" in {
      ...
      val result =
        connectionsRepository
          .revokeConnection(acceptor, connectionId)
          .run(TraceId.generateYOLO)
          .unsafeRunSync()
      result.isRight must be(true)
    }
    ...
    "delete the messages related to the connection" in {
      ...
      MessagesDAO
        .getConnectionMessages(initiator, connectionId)
        .unsafeRun()
        .isEmpty must be(true)
      MessagesDAO
        .getConnectionMessages(acceptor, connectionId)
        .unsafeRun()
        .isEmpty must be(true)
    }
    ...
    ConnectionStatus.values
      .filterNot(_ == ConnectionStatus.ConnectionAccepted)
      .foreach { status =>
  ...
  "getConnection" should {
    "returns the connection when the initiator asks for it" in {
      ...
      val connectionId: ConnectionId =
        createConnection(h1, h2, token, ConnectionStatus.ConnectionAccepted)
  ...
  "getConnectionsPaginated" should {
    ...
    def createExampleConnections(
        verifierId: ParticipantId,
        zeroTime: Long
    ): Map[String, ConnectionId] = {
      (for (i <- 0 to 12) yield {
        ...
        val issuerId = createIssuer(
          issuerName,
          Some(ParticipantLogo(Vector(10.toByte, 15.toByte)))
        )
  ...
  "getConnectionByToken" should {
    "return correct token and connectionId" in {
      ...
      val token = createToken(h1)
      val connectionId: ConnectionId =
        createConnection(h1, h2, token, ConnectionStatus.InvitationMissing)
  ...
  "getConnectionStatuses" should {
    "return correct list of connection statuses" in {
      ...
      val contactConnection1 = ContactConnection(
        Some(connectionId1),
        Some(token1),
        ConnectionStatus.InvitationMissing
      )
      val contactConnection2 = ContactConnection(
        Some(connectionId2),
        Some(token2),
        ConnectionStatus.ConnectionAccepted
      )
      ...
      val connectionStatuses =
        connectionsRepository
          .getConnectionsByConnectionTokens(
            List(TokenString("tokenString1"), TokenString("tokenString2"))
          )
          .run(TraceId.generateYOLO)
          .unsafeToFuture()
          .futureValue
