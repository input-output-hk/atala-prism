...
trait ConnectionsService[F[_]] {
  ...
  def getConnectionCommunicationKeys(
      connectionId: ConnectionId,
      userId: ParticipantId
  ): F[Either[ConnectorError, Seq[(String, ECPublicKey)]]]
...
private class ConnectionsServiceImpl[F[_]: MonadThrow](
  ...
  def getConnectionCommunicationKeys(
    ...
    def getDidCommunicationKeys(did: DID): F[Either[ConnectorError, Seq[(String, ECPublicKey)]]] = {
      ...
      result.map[Either[ConnectorError, Seq[(String, ECPublicKey)]]](Right(_)).recover { case ex =>
        Left(InternalServerError(ex))
      }
    }
