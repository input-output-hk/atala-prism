...
\begin{proof}[\thmref{thm:anonymity-uas}; Anonymity of \CUASGen]
  ...
  \begin{minipage}[t]{\textwidth}
    \procedure{$\SimSetup(\secpar,\AttrSpace)$}{%
      ...
      \textrm{Compute}~\Cparm,\SBCMparm,\Sparm,\Eparm~\textrm{as in}~\Setup \\
    ...
    \begin{minipage}[t]{\textwidth}
      \begin{minipage}[t]{0.5\textwidth}
        ...
        \end{minipage}
        \begin{minipage}[t]{0.5\textwidth}
          \procedure{$SimObt(\attrs,\sipk_{\scred})$}{%
            \textrm{Runs \SBCM}
            \textrm{Runs like \Obtain, replacing calls to
            $\NIZKProve^{\NIZKRel_{\Issue}}(\NIZKcrs_{\Issue},
            (\Ccom,\attrs,\sipk_{\scred}),
            (\usk,\scred,\attrs_{\scred},r))$ with calls to
            $\NIZKSim^{\NIZKRel_{\Issue}}(\NIZKcrs_{\Issue},\NIZKtrap_{\Issue},
            (\Ccom,\attrs,\sipk_{\scred}))$}
        }
        \end{minipage}
      \end{minipage}
