...
\begin{figure}[htp!]
  \procedure[linenumbering]{$\ExpGSACTrace(1^\secpar)$}{%
    ...
    \textrm{Let}~(\upk,\oproof) \gets \Open(\gpk,\osk,\trans,\sig,\DAttrs,\msg) \\
    \pcif \Judge(\gpk,\upk,\oproof,\sig,\DAttrs,\msg) = 0: \pcreturn 1 \\
    \pcelse \pcif \forall \uid \in \CU, \nexists (\uid,\cdot,\Attrs) \in
    \CRED~\st~\DAttrs \subseteq \Attrs: \pcreturn 1 \\
    \pcreturn 0
  }
...
\begin{figure}[htp!]
  \procedure[linenumbering]{$\ExpGSACNonframe(1^\secpar)$}{%
    \parm \gets \Setup(1^\secpar) \\
     (\ipk,\isk) \gets \IKeyGen(\parm);~(\opk,\osk) \gets \OKeyGen(\parm);~ 
     \gpk \gets (\ipk,\opk) \\
     (\sig,\DAttrs,\msg,\upk,\oproof) \gets
     ...
     \Judge(\gpk,\upk,\oproof,\sig,\DAttrs,\msg) = 1~\land \\
