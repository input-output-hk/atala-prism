...
\begin{figure}[htp!]
  \procedure[linenumbering]{$\ExpGSACSigAnonb(1^\secpar)$}{%
    \parm \gets \Setup(1^\secpar) \\
    (\ipk,\isk) \gets \IKeyGen(\parm);~(\opk,\osk) \gets \OKeyGen(\parm);~ 
    \gpk \gets (\ipk,\opk) \\
    b^* \gets
    \adv^{\HUGEN,\CUGEN,\WREG,\OBTAIN,\SIGN,\OPEN,\OBTCHALb}(\gpk,\isk,\status) \\
    \pcreturn b^*
  }
  ...
  \caption{Issuance and signature anonymity experiments for \GSAC schemes.}
  \label{fig:exp-gsac-anonb}
\end{figure}
 
\begin{definition}{(Issuance anonymity of \GSAC)}
  \label{def:iss-anonymity-gsac}
  We define the advantage \AdvGSACIssAnon of $\adv$ against \ExpGSACIssAnonb as
  $\AdvGSACIssAnon=|\Pr\lbrack\ExpGSACIssAnono(1^\secpar)=1\rbrack-
  \Pr\lbrack\ExpGSACIssAnonz(1^\secpar)=1\rbrack|$.
  %
  A \GSAC scheme satisfies issuance anonymity if, for any p.p.t. adversary
  $\adv$, \AdvGSACIssAnon is a negligible function of $1^\secpar$.
\end{definition}

\begin{definition}{(Signature anonymity of \GSAC)}
  \label{def:sig-anonymity-gsac}
  We define the advantage \AdvGSACSigAnon of $\adv$ against \ExpGSACSigAnonb as
  $\AdvGSACSigAnon=|\Pr\lbrack\ExpGSACSigAnono(1^\secpar)=1\rbrack-
  \Pr\lbrack\ExpGSACSigAnonz(1^\secpar)=1\rbrack|$.
  %
  A \GSAC scheme satisfies signature anonymity if, for any p.p.t. adversary
  $\adv$, \AdvGSACSigAnon is a negligible function of $1^\secpar$.
\end{definition}
