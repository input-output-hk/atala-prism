...
\jesus{Adding a \uid into \HU or \CU within an oracle that is executed by end
  users in the real world is a bit weird if not clarified. Some arguments may
  be possible.}
...
\todo{Clarify that, for us, issuers and users are essentially the same thing.}
...
Concerning \fissue functions, our \UAS model requires them to be \UAS-acceptable
...
\begin{figure}[ht!]
  ...
  \scalebox{0.9}{
    \procedure{$\Exp^{\UAS-acc-b}_\adv(1^\secpar)$}{%
      ...
      \gets \adv(\upk) \\
      \pcif b=0:
      \yissue^* \gets \fissue(\upk,\usk,\lbrace (\cidi,\cattrs_i)
...
\todo{Clarify that {\iid}s are just {\uid}s.}
  ...
  \begin{figure*}[htp!]
    ...
    \scalebox{0.9}{
      ...
      \begin{minipage}[t]{0.55\textwidth}
        ...
        \procedure{$\OBTISS(\cid=(\cidi,\cidu),\uid,\iid,\attrs,$\\
          \hspace*{4.25em}$\lbrace (\cid_i,\iid_i) \rbrace_{i \in [n]})$}{%
          \pcif \uid \notin \HU \lor \iid \notin \HI: \pcreturn \bot \\
    ...
    \scalebox{0.9}{
      ...
      \begin{minipage}[t]{.5\textwidth}
        ...
        \procedure{$\OBTCHALb(\cidu,\cuid_{0,1},\iid,\attrs,\cscid_{0,1}=$\\
          \hspace*{5.25em}$\lbrace (\ccid_{0,i},\ccid_{1,i}) \rbrace_{i\in[n]})$}{%
          \pcif \cuid_0 \notin \HU \lor \cuid_1 \notin \HU: \pcreturn \bot \\
          \pcif \iid \notin \CI: \pcreturn \bot \\
          \pcif \CRED[(\cdot,\cidu)] \neq \bot \lor
          \CCRED[(\cdot,\cidu)] \neq \bot: \pcreturn \bot \\
          ...
          \pcif 0(\fissue(\PUBUK[\cuid_0],\PRVUK[\cuid_0],\cscid_0,\attrs)) \neq \\
          \pcind 0(\fissue(\PUBUK[\cuid_1],\PRVUK[\cuid_1],\cscid_1,\attrs)):
          \pcreturn \bot \\
          \langle \Cred, \cdot \rangle \gets
        ...
        \procedure{$\CHALb(\oid,\cscid_{0,1}=
          ...
          \feval) \\
          \cSig_{1-b} \gets \Sign(\PRVUK[\cuid_{1-b}],\PUBOK[\oid],\cscid_{1-b},\msg,\feval) \\          
          \CSIG[\cSig_b] \gets 
...
%      \fissue^{\cid}),\cdot)\\
      ...
      \pcind \pcif \fissue(\PUBUK[\uid],\PRVUK[\uid],\attrs,
...
\todo{All the following assumes \UAS-acceptable \fissue functions.}
...
\todo{Mention that, in this definition, a credential that does not meet \fissue
  is not considered a forgery unless it is used to produce a signature. This is
  consistent with other works -- e.g., Camenisch, Krenn, Lehmann...}
\todo{Require that $n>=1$ in the creds used to produce a signature...}
...
%       \vspace*{0.5em}
	...
	(\upk,\usk,\lbrace(\cid_i,\attrs_i,\cdot)\rbrace_{i\in[n]},
        \yeval^1,\yinsp') \gets \ExtractSign(\trap,\oid,\siid,\Sig,\msg,\feval) \\
        \pcif \exists i\in[n]~\suchthat~
        \fissue(\upk,\usk,\attrs_i,\lbrace(\cid_{i,j},\attrs_{i,j})
        \rbrace_{j\in[n_i]}) \neq \yissue^i \lor
        \OWNR[\lbrace\cid_{i,j}\rbrace_{j\in[n_i]}]\notin \HU \cup \CU, \\
...
\todo{Need to make the $\cid_i$s explicity in the \SBCMVerify clauses of
  $\NIZK^{\Issue}$ and $\NIZK^{\Sign}$.}
\begin{figure}[ht!]
  \centering
  \scalebox{0.9}{
...
\begin{theorem}[Unforgeability of \CUASGen]
  ...
  satisfies unforgeability as defined in \defref{def:sign-forge-uas}, except
...
\todo{Security proofs are deferred to \appref{app:uas-variants-proofs}.}
...
% systems. In both cases, users may end up acting as issuers themselves. The
% algorithms are as in \CUASGen, except that there is no \UKeyGen -- or, rather,
% the steps run by \UKeyGen are exactly the same as in \IKeyGen. Concretely,
% $\usk \gets \isk$ and $\upk \gets \ipk$, where \isk and \ipk are generated
% as in \IKeyGen.
...
\paragraph{Interactive Credential Presentations.}
\todo{Does this meet the model? If not, we should move this elsewhere.}
In \appref{app:interactive-uas}
