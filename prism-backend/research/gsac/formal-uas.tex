...
function. Moreover, any credential used to support the request needs to belong
to a known user. Without conventional opening, we need to resort to extraction.
...
Signature unforgeability is simply a combination of known requirements in AC and
...
(\yeval^0,\yeval^1)$ value; or (3), cannot be associated to known credentials
owned by a known user.

The respective properties, \ExpForgeIssue and \ExpForgeSign, are formally
defined in \figref{fig:exp-uas-unfor}).
