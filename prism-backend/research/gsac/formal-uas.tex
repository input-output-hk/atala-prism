...
\begin{description}
\item[$\fissue: (\usk,\attrs,\lbrace (\cid_i,\cattrs_i) \rbrace_{i \in [n]})
  \rightarrow 0/1$.] Chosen
  ...
  over attributes \attrs, when requested by a user with secret key \usk,
...
\item[$\feval: (\usk,\lbrace(\cid_i,\cattrs_i)\rbrace_{i \in [n]},\msg)
  ...
  receive the user secret key \usk, a set of credentials with identifiers and
  attributes given by $\lbrace (\cid_i,\cattrs_i) \rbrace_{i \in [n]}$ (where
  $n \ge 1$), and the message to be signed \msg. \feval outputs two values,
  $\yeval^0$ and $\yeval^1$, which can be used to modulate the bevahiour of
  \finsp, and must belong in a well defined set \rngfeval. For readability, we
  write $\Yeval = (\yeval^0,\yeval^1)$.
\item[$\finsp: (\Yeval,\usk,\lbrace \cid_i,\cattrs_i\rbrace_{i \in [n]},\msg)
  \rightarrow \yinsp$.]
...
\begin{definition}{(\UAS-acceptable functions)}
  ...
  is one-way with respect to \usk. That is, given the output of \fissue
  (similarly, \feval or \finsp), it is at least computationally unfeasible to
  recover the \usk value used to produce it.
\end{definition}
    ...
    \scalebox{0.9}{
      ...
      \begin{minipage}[t]{0.55\textwidth}
        ...
        \procedure{$\OBTISS(\cid,\uid,\iid,\attrs,\lbrace (\cid_i,\iid_i)
          ...
          \langle \Obtain(\UK[\uid],\CRED[\lbrace \cid_i\rbrace_{[n]}],\attrs), \\
          \hspace*{60pt} \Issue(\PRVIK[\iid],\ISR[\lbrace \cid_i\rbrace_{[n]}],\attrs)
          \rangle \\
...
\item[$\SimSetup(1^\secpar) \rightarrow (\parm,\trap)$.] Given a security
  ...
  trapdoor \trap.
\item[$\ExtractIssue(\trap,\utrans) \rightarrow (\usk,\sCred=
...
\item[$\ExtractSign(\trap,\oid,\lbrace \iid_i \rbrace_{i \in [n]},\Sig,\msg,
  \feval) \rightarrow (\usk,\sCred=\lbrace(\cid_i,\attrs_i,\cred_i)
...
non-frameability. In addition, we define two auxiliary properties for defining
  ...
  \scalebox{0.9}{
    \begin{minipage}[t]{0.57\textwidth}
      \procedure[linenumbering]{$\ExpForgeIssue(1^\secpar)$}{%
        ...
        (\usk,\lbrace(\cid_i,\attrs_i,\cdot)\rbrace_{i \in [n]}) \\
        \hspace*{5em}\gets \ExtractIssue(\trap,\trans[\cid]) \\
        % \pcfor i \in [n]: (\usk_i,\cdot) \gets \ExtractIssue(\trap,
        % \trans[\cid_i]) \\
        % \pcif \exists i \in [n]~\suchthat~\usk \neq \usk_i: \pcreturn 1 \\
        \pcif \fissue(\usk,\ATTR[\cid],\lbrace (\cid_i,\attrs_i)
        \rbrace_{i\in[n]}) = 0: \\
        \pcind \pcreturn 1 \\
	...
	(\usk,\lbrace(\cid_i,\attrs_i,\cdot)\rbrace_{i\in[n]},
        \yeval^1,\yinsp') \gets \ExtractSign(\trap,\oid,\siid,\Sig,\msg,\feval) \\
        (\yeval^0,\yeval^1) \gets \feval(\usk,\lbrace(\cid_i,\attrs_i)
        \rbrace_{i\in[n]}, \msg) \\
        \pcif \yeval \neq \yeval^0 \lor \yeval^1 \neq \tyeval^1:
        \pcreturn 1 \\
        \pcif \finsp((\yeval^0,\yeval^1),\usk,\lbrace(\cid_i,\attrs_i)
        \rbrace_{i\in[n]},\msg)
        \neq \yinsp \lor \yinsp \neq \yinsp':
        \pcreturn 1 \\
        \pcif \OWNR[\scid] \notin \HU \cup \CU: \pcreturn 1 \\
...
\begin{figure}[htp!]  
  ...
  \scalebox{0.9}{
    \procedure[linenumbering]{$\ExpNonframe(1^\secpar)$}{%
      ...
      \pcreturn 0 \\
      (\usk,\lbrace(\cid_i,\attrs_i,\cdot)\rbrace_{i\in[n]},\tyeval^1,\yinsp') \gets
      \ExtractSign(\trap,\oid,\siid,\Sig,\msg,\feval) \\
      % \Yeval=(\yeval^0,\yeval^1) \gets \feval(\usk,\scred,\msg) \\
      % \pcif \yeval \neq \yeval^0 \lor \tyeval^1 \neq \yeval^1: \pcreturn 1 \\
      % \pcif \finsp(\Yeval,\usk,\scred,\msg) \neq \yinsp
      % \lor \yinsp \neq \yinsp': \pcreturn 1 \\
      \Yeval=(\yeval^0,\yeval^1) \gets \feval(\usk,\lbrace (\cid_i,
...
each $\cred_i$ is issued the corresponding issuer with public key $\ipk_i$, and
...
$\cred_i$ credentials and its user secret key. For this, we use relation
$\NIZKRel_{\Issue}$, as defined above. The interactive protocol for a user to
obtain a credential from an issuer of the system is then simply an execution of
the interactive signing
protocol of an \SBCM scheme, where the user runs $\SBCMCom(\ipk,\usk,\lbrace,
\cid, \attrs \rbrace)$, and the issuer runs $\SBCMSign(\isk,\lbrace \cid, \attrs
\rbrace)$, for some commonly agreed and fresh \cid, and using
...
the issuer is its transcript, which is a $(\Ccom,\lbrace \cid, \attrs\rbrace,
\lbrace \ipk_i\rbrace_{i\in[n]},\cred,\pi)$ tuple.
