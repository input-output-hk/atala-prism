...
\begin{description}
\item[$\fissue: (\upk,\attrs,\lbrace (\cid_i,\cattrs_i)
  \rbrace_{i \in [n]}) \rightarrow \yissue$.] Chosen
    ...
    \scalebox{0.9}{
      ...
      \begin{minipage}[t]{.5\textwidth}
        ...
        \procedure{$\OBTCHALb(\cidu,\cuid_{0,1},\iid,\attrs,\cscid_{0,1}=$\\
          ...
          \pcif 0(\fissue(\PUBUK[\cuid_0],\cscid_0,\attrs)) \neq \\
          \pcind 0(\fissue(\PUBUK[\cuid_1],\cscid_1,\attrs)): \pcreturn \bot \\
          \langle \Cred, \cdot \rangle \gets
        ...
        \procedure{$\CHALb(\cuid_{0,1},\oid,\cscid_{0,1}=$\\
          \hspace*{5em}$\lbrace (\ccid_{0,i},\ccid_{1,i})
...
\begin{figure}[htp!]
  ...
  \scalebox{0.9}{
    \procedure[linenumbering]{$\ExpCorrect(1^\secpar)$}{%
      ...
      \gets \adv^{\UGEN,\OGEN,\ISET,\OBTISS,\RREG}(\parm) \\
...
%      \fissue^{\cid}),\cdot)\\
      ...
      \pcind \pcif \fissue(\PUBUK[\uid],\attrs,
...
\begin{figure}[ht!]
  ...
  \todo{Is $\C = \CCommit(\usk;r)$ at Issue needed now that \upk is added?}}
  \label{fig:nizkrels}
\end{figure}

From them, we build \CUASGen as follows.
\todo{Check if \SSetup etc is still needed...}
...
\NIZKcrs_{\Open})$. % Many of these parameter setup algorithms can
...
Each user first parses \parm as $(\cdot,\SBCMparm,
\cdot,\cdot,\cdot,\cdot,\cdot,\cdot)$. Then, runs $(\SBCMvk,\SBCMsk) \gets
\SBCMKeyGen(\SBCMparm)$, $\upk \gets (\parm,\SBCMvk)$, $\usk \gets \SBCMsk$,
and returns $(\upk,\usk)$.
...
the issuer based on the $\cred_i$ credentials and its user key pair.
...
issuer first needs to send \cidi to the user.
Then both issuer and user interact to produce the \SBCM signature, where the
...
communication transcript, which is a $(\Ccom,(\cid,\attrs),\lbrace
\ipk_i \rbrace_{i\in[n]},\cred,\pi)$ tuple.
...
schemes. \todo{Security proofs are deferred to \appref{app:uas-variants-proofs}.}
