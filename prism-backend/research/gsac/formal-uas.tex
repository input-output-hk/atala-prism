...
This may seem too ideal to model real world situations. However, we argue that
this can in fact be very close to reality, where a well-known issuer, after
properly identifying a user, attests the (possibly blinded) public key this
user proves control of. Indeed, our model and syntax support exactly this
approach, by allowing users to leverage previously obtained credentials in
order to request new ones.
...
%
All tables/sets are initialized as empty tables/sets. For notational
convenience, we often use these identifiers instead of the corresponding
user/issuer/opener keys.
%
We use the following sets and tables in our definitions:
...
\item[\CCRED.] Like \CRED, but tracks challenge credentials in the issuance
  anonymity game.
...
(provided the previous constraints) when he gets challenge credentials
...
have different privacy levels. For instance, a scheme that has very granular
...
(1) the produced signature leveraged with a credential for
...
$\NIZKRel_{\Issue}$, $\NIZKRel_{\Sign}$, and $\NIZKRel_{\Open}$, described next
...
\begin{theorem}[Unforgeability of \CUASGen]
  ...
  extractable, the scheme for $\NIZKRel_{\Open}$ is complete, the public key
  encryption algorithm is correct, and the \SBCM scheme is correct and one-more
  unforgeable, then our \CUASGen construction
...
\begin{theorem}[Non-frameability of \CUASGen]
  ...
  extractable, the scheme for $\NIZK^{\Open}$ is simulation-extractable, and the
...
variant next $\CUASGen^{hide-iss}$, that we leverage to build related schemes.
%
Similarly, we describe $\CUASGen^{int}$ which transforms \CUASGen by turning the
...
We sketch security of $\CUASGen^{int}$ in \appref{sapp:sec-interactive} assuming
