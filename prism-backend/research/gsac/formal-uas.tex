...
\begin{description}
\item[$\fissue: (\usk,\attrs,\attrs_{\scred}) \rightarrow 0/1$.] Chosen
  ...
  potentially leveraging a set of previously obtained credentials with
  attributes $\attrs_\scred$. \fissue returns $1$ to accept a request, $0$ to
  reject it.
\item[$\feval: (\usk,\attrs_{\scred},\msg)
  ...
  credential used for signing, and message to be signed
...
non-frameability. In addition, we make use of some helper functions:
\ExtractIssue and \ExtractSign, which let us recover the witnesses used to
produce a credential or a signautre; and \IdentifyCred, and \IdentifySig, which
associate credentials and signatures to user identifiers \uid. Both types of
functions have been used before. For instance, the extraction type are used in
...
wins if it manages to get a credential from an honest user, such that the
issuance predicate \fissue rejects the extracted witnesses.
%
...
honest issuers are required, as we rely on the availability of $\langle \Obtain,
\Issue \rangle$ transcripts to assist on the extraction. On the other hand,
for \ExpExtractSign, as we extract directly from signatures, honest issuers are
not a requirement, so the adversary is given access to $\ICORR$ too.
...
\begin{figure}[htp!]
    \procedure[linenumbering]{$\ExpForgeIssue(1^\secpar)$}{%
      ...
      \IK[\iid]~\textrm{as}~((\cdot,\fissue),\cdot) \\
      (\usk,\scred,\attrs_{\scred}) \gets \ExtractIssue(\trap,\trans[\cid]) \\
      \pccomment{\ExpIdentifyCred enforces the same \uid is
        returned for all \scred} \\
...
\begin{definition}{(Unforgeable issuance of \UAS)}
  ...
  %
  A \UAS scheme \emph{with identifiable credentials} has unforgeable issuance
  if, for any p.p.t. adversary $\adv$, \AdvForgeIssue is a negligible function
  of $1^\secpar$.
\end{definition}
...
\begin{definition}{(Unforgeable signing of \UAS)}
  ...
  %
  A \UAS scheme \emph{with identifiable signatures} has unforgeable signing if,
  for any p.p.t. adversary $\adv$, \AdvForgeSign is a negligible function of
  $1^\secpar$.
\end{definition}
