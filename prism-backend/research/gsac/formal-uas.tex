...
information extractable from a signature. Signing and verification are
non-interactive processes like in GS schemes, but we show how to convert to an
interactive one as in ACs. Finally, we incorporate functions that allow
tweaking the conditions checked at credential issuance and signature generation
times, including the information that an opener will be able to extract from a
signature. This flexibilizes the scheme even beyond the loose relationship
between issuer and opener.
...
\begin{figure}[htp!]
    \procedure[linenumbering]{$\ExpForgeIssue(1^\secpar)$}{%
      ...
      \pccomment{\todo{New! Update proofs.}} \\
      \pcreturn 0
    }
