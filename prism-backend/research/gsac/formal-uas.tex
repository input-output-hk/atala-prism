...
\item[$\ExtractIssue(\trap,\utrans) \rightarrow (\upk,\usk,\sCred=
  ...
  and, if any, the credential set \sCred used to request \Cred.
...
allowing to support credential requests with previously obtained credentials --
functionality which, to the best of our knowledge, \UAS is the first scheme
to support. In both cases, it is necessary to assume honest issuers. 

Signature unforgeability, \ExpForgeSign, is mainly a combination (and
...
credentials owned by a known user. Here, we again need to assume honest issuers.
...
While it may seem enough to check that no valid signature can be created from
forged credentials (i.e., credentials that did not meet their issuance policy,
or are not bound to a known user), note that allowing credentials to
endorse further credential requests makes this argument insufficient. For
instance, if we only check the credentials involved in signature generation, it
is perfectly possible that a forged credential is used to obtain a legitimate
one, and then use the latter to finally create a valid signature -- concretly,
such misbehaviour is not prevented by \ExpForgeSign. 
...
involved in the request is bound to the same known user. For this, we allow the
adversary \adv~to interact with the oracles (restricted to honest issuers).
Eventually, \adv~is challenged to output a credential identifier from which we
extract the actual witnesses used in the credential request associated to \cid.
\adv~wins if the issuance function, applied to the extracted values, does not
meet the expected output, or if not all of the credentials (if any) used to
support the request belong to a same (known) user. Note that the issuance
transcript associated to \cid must exist, as we unavoidably need to restrict to
honest issuers. Indeed, a dishonest issuer cannot be trusted to enforce any
policy, nor to properly keep all transcripts. In practice, this means that
dishonest issuers can create arbitrary credentials.

We formalize both properties in \figref{fig:exp-uas-unfor}.
  ...
  \scalebox{0.85}{
    \begin{minipage}[t]{0.6\textwidth}
      \procedure[linenumbering]{$\ExpForgeIssue(1^\secpar)$}{%
        (\parm,\trap) \gets \SimSetup(1^\secpar) \\
        % (\Cred,\uid,\iid,\oid,\feval,\msg) \gets \adv^{\Oforge}(\parm) \\
        % \Sig \gets \Sign(\PRVUK[\uid],\PUBOK[\oid],\Cred,\msg,\feval) \\
        % \pcif \Verify(\PUBOK[\oid],\PUBIK[\iid],\Sig,\msg,\feval) = 0: \\
        % \pcind \pcreturn 0 \\
        % (\upk,\usk, (\cid,\attrs,\cdot), \cdot, \cdot) \\
        % \pcind \gets \ExtractSign(\trap,\oid,\iid,\Sig,\msg,\feval) \\
        % \pcif \trans[\cid] = \bot \lor \CRED[\cid] = \bot: \pcreturn 1 \\
        % (\yissue,\cdot) \gets \trans[\cid];
        % ((\cdot,\fissue),\cdot) \gets \PUBIK[\cid] \\
        % \pcif \nexists \lbrace (\cid_i,\attrs_i) \rbrace_{i\in[n]}
        % \in \CRED[\uid]~\suchthat \\
        % \pcind \fissue(\upk,\cid,\attrs,
        % \lbrace (\cid_i,\attrs_i) \rbrace_{i\in[n]}) = \yissue: \\
        % \pcind \pcreturn 1
        % 
        % ((\upk_1,r_1,\Ccom_1),\dots,(\upk_l,r_l,\Ccom_l),\st) \\
        % \pcind \gets \adv^{\HUGEN,\CUGEN}(\texttt{choose},\parm) \\
        % \pcif \exists i\in[l]~\suchthat~\forall \uid,
        % \upk_i \neq \PUBUK[\uid]:  \\
        % \pcind \pcreturn 0 \\
        % \cid \gets \adv^{\Oforge}(\texttt{forge},\st,\parm) \\
        \cid \gets \adv^{\Oforge}(\parm) \\
        \pcif \ISR[\cid] \notin \HI \lor \trans[\cid] = \bot: \pcreturn 0 \\
        (\utrans,\yissue) \gets \trans[\cid] \\
        (\upk,\cdot,\lbrace (\cid_i,\attrs_i) \rbrace_{i\in[n]},\cdot,\attrs,
        \cdot) \gets \ExtractIssue(\trap,\utrans) \\
    ...
    \begin{minipage}[t]{0.5\textwidth}
      \procedure[linenumbering=on]{$\ExpForgeSign(1^\secpar)$}{%
        ...
        \exists \uid~\suchthat~(\cdot,\cdot,\Sig,\cdot,\msg,\feval) \in
        \SIG[\uid]~\lor  \\
...
is essentially a sequence of \SBCMBlind, \SBCMSign, and \SBCMUnblind runs
...
\item \underline{Issuer}: Run $\SBCMbsig \gets \SBCMSign^{\RelIss}(\isk,\Ccom,
  \pi,\attrs;\yissue)$. Return \SBCMbsig to the user. Output $((\cidi,\Ccom,\pi,
  \SBCMbsig),\yissue)$. 
\item \underline{User}: Run $\SBCMsig \gets \SBCMUnblind^{\RelIss}(\ipk,
  \SBCMbsig,\Ccom,r,\usk,\lbrace \cid \rbrace \cup \attrs; \yissue, \lbrace
  (\cid_i,\cattrs_i)\rbrace_{i\in[n]})$. Output $\cred \gets \SBCMsig$.
\end{itemize}
...
\begin{theorem}[Issuance unforgeability of \CUASGen]
  ...
  underlying \SBCM scheme is correct, then our \CUASGen construction satisfies
