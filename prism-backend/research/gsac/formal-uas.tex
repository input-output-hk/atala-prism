...
%      \fissue^{\cid}),\cdot)\\
      ...
      \pcind \pcif \fissue(\UK[\uid],\attrs,
      (\lbrace(\cid_i,\ATTR[\cid_i])\rbrace_{i\in[n]})) = 0: \pcreturn 1 \\
      (\yeval^0,\yeval^1) \gets \feval(\UK[\uid],(\scid,\ATTR[\scid]),\msg) \\
      \pcif \yeval^0 \neq \yeval: \pcreturn 1 \\
  ...
  \scalebox{0.9}{
    \begin{minipage}[t]{\textwidth}
      \procedure[linenumbering]{$\ExpForgeIssue(1^\secpar)$}{%
        ...
        \gets \ExtractIssue(\trap,\trans[\cid]) \\
        % \pcfor i \in [n]: (\usk_i,\cdot) \gets \ExtractIssue(\trap,
        % \trans[\cid_i]) \\
        % \pcif \exists i \in [n]~\suchthat~\usk \neq \usk_i: \pcreturn 1 \\
