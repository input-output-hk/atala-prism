...
\todo{Clarify how $\fissue^{single,F,I}$ works! This is also common with other
  constructions...}
...
Observe that the $\fissue^{single,F,I}$ function is further parameterized with
...
Note that, in the $\feval^{\dattrs,F,V,L}$ function, $L$ is a sort of variable.
However, each verifier $V$ may just update its own list with some reasonable
frequency (e.g., daily), and use a pre-compiled version of
$\feval^{\dattrs,F,V,L}$ in which $L$ is actually a constant.
%
...
\finsp^\ast)$-restriction of \CUASGen with only one issuer and opener, one
credential per user, and one attribute per credential (the user secret key),
implements an MPS scheme where $F=\feval^{\yeval^1}$ and $G=\finsp^\ast$.
With these functions, the construction is essentially as in
