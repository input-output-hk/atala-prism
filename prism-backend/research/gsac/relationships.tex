  ...
  \scalebox{0.9}{
    \begin{minipage}[t]{\textwidth}
      ...
      \begin{align}
        ...
        & \fissue^{single,L}(\upk,\cdot,\cdot) \coloneqq
          \pcreturn \upk \notin L
          \nonumber \\
...
of our $\CUASGen^{hide-iss}$ construction. The construction is as follows:
...
\item[$((\pk_1,\sk_1),...,(\pk_n,\sk_n)) \gets KeyGen(\secpar)$.] Runs
  ...
  instances of $(\ipk_i,\isk_i) \gets \IKeyGen(\parm,\fissue^{\sring})$, where
  $\sring = \lbrace \ipk_i \rbrace_{i\in[n]}$, and $(\opk,\osk) \gets
  \OKeyGen(\parm,\finsp^0)$. 
\item[$\langle \cred,\bot,\utrans,\bot \rangle \gets
  \langle \Obtain(\usk,\scred,\attrs),\Issue(\isk,\sipk=\attrs,\attrs)\rangle.$]
  A user, with $\usk=\isk_i$ and $\upk=\ipk_i$ for some $i \in [n]$, acts both
  as user and issuer to self-issue a credential for an attribute set \attrs such
  that $\attrs \subseteq \sring$, where \sring is the set of public keys used to
  define $\fissue^{\sring}$.
\item[$(\sig,\yeval) \gets \Sign(\usk,\opk,\cred,\msg,\feval).$] As in
  $\CUASGen^{hide-iss}$, using $\feval^{\sring,\dattrs}$ as evaluation
  function, using as signing credential the self-issued credential. Note that
  $\dattrs \subseteq \attrs = \sring$ defines the subring to use for the
  produced signature.
\item[$1/0 \gets \Verify(\opk,\sipk,\sig,\yeval,\msg,\feval).$] As in
  $\CUASGen^{hide-iss}$, making $\sipk=\yeval=\lbrace \ipk_i
  \rbrace_{i\in\dattrs}$.
\end{description}
