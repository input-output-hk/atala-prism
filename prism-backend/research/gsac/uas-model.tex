...
\begin{figure}[htp!]
  \procedure{$\ExpAnonb(1^\secpar)$}{%
     ...
     \adv^{\lbrace\CI,\CO,\HU,\CU\rbrace\GEN,\OBTAIN,\SIGN,\INSPECT}
     (\choose,\parm) \\
     b^* \gets
     \adv^{\lbrace\CI,\CO,\HU,\CU\rbrace,\GEN,\OBTAIN,\SIGN,\INSPECT,\CHALb}
     (\guess,\status) \\
     \pcreturn b^*
  }
...
\begin{figure}[htp!]
  \procedure{$\ExpNonframe(1^\secpar)$}{%
    \parm \gets \Setup(1^\secpar) \\
     ...
     \pcind \nexists \uid \in \HU~\st~\sig \in \SIG[\uid]~\land
     \pccomment{No honest user produced \sig} \\
%     \pcind \forall \uid \in \CU~\Identify(\sgid,\uid,\sig) = 0~\land
%     \pccomment{No corrupt user secret key was used to produce \sig } \\
     \
     \pcind \exists \uid' \in \HU~\st~\y = \finsp(\PUBUK[\uid'],\CRED[\uid'],\sig)
     \pccomment{\finsp outputs \y for some honest user and \sig} \\
     \pcreturn 0
  }
