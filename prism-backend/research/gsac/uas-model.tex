...
{%\setlength\intextsep{\sep}
  \begin{figure*}[htp!]
    ...
    \caption{Detailed oracles available in our model (1/2). Oracles for
      generating users, issuers, and inspectors.\\
      \doubt{Do we need to specify the functions in the GEN Oracles? They do
        not seem necessary in \SIGN/\CHALb/\INSPECT. Should they? Maybe there
        is something wrong here... Maybe they are needed in the games...}}
    \label{fig:oracles1}
  \end{figure*}
}
    ...
    \scalebox{0.9}{
      ...
      \begin{minipage}[t]{.5\textwidth}
        ...
        \procedure{$\OBTISS(\cid,\uid,\gid,\uattrs,\dattrs,
          ...
          \langle \Obtain(\PRVUK[\uid],\uattrs,\dattrs,\ldblbrace (\GK[\sgid],
          \CRED[\scid],\tuattrs,\tdattrs) \rdblbrace), \\
          \pcind \pcind \pcind \pcind \pcind \pcind
          \Issue(\PRVIK[\gid],\PUBUK[\uid],\ldblbrace (\GK[\sgid],\tdattrs)
          \rdblbrace ) \rangle \\
        ...
        \procedure{$\CHALb(\msg,
          ...
          \pcif \cuid_0 \notin \HU \lor \cuid_1 \notin \HU: \pcreturn \bot \\
          \pccomment{Note: since both \uid's need to be honest, we must have
            all the}\\
          \pccomment{undisclosed attributes in the \ATTR lists.} \\
          \dattrs' \gets \DATTR[\cscid_0] \cap \DATTR[\cscid_1] \\
          \uattrs' \gets \UATTR[\cscid_0] \cap \UATTR[\cscid_1] \\
          \pcif (\dattrs_0 \cap \dattrs_1) \nsubseteq \dattrs':
          \pcreturn \bot \\
          \pcif (\uattrs_0 \cap \uattrs_1) \nsubseteq \uattrs':          
          \pcreturn \bot \\
          \csig \gets \Sign(\PRVUK[\uid_b],\msg,\uattrs_b,\dattrs_b,
          \ldblbrace (\GK[\sgid],\CRED[\scid_b],\tuattrs_b,\tdattrs_b) \rdblbrace) \\
          \CSIG \gets \CSIG \cup
          \lbrace (\csig,\msg,\scid_b,\uattrs_b,\dattrs_b,\sgid) \rbrace \\
          \pcreturn \csig
        }
