...
\todo{Formal models on signatures over blocks of committed messages? I'm just
  informally stating a model here, but there must be some more formal
  reference... \cite{cl02,asm06,ps16} all kind of allow signatures on blocks of
  (committed) messages, but they seem to use ad hoc models.}

We use signature schemes that allow signing messages and commitments to messages
...
\secref{sapp:commitments}). The algorithms composing an \SBCM scheme are as
follows:
...
\todo{BBS+ is proven to have EUF, but nothing is said against OMF. Relationship?}
