...
\todo{Commitments with proofs, needed for \SBCM.}
...
\item[$\SBCMsig \gets \langle \SBCMCom(\overline{\smsg},\smsg),
...
\begin{figure}[ht!]
  \begin{minipage}[t]{0.5\textwidth}
    \procedure{$\ExpSBCMEUF(\Ssecpar)$}{%
      ...
      \pcif \SVerify(\SBCMvk,\SBCMsig,\smsg) = 0: \pcreturn 0 \\
      \pcif \nexists (\smsg',\sCcom=\CCommit(\Cparm,\smsg''))~\st~
      (\lbrace \smgs' \rbrace \cup \lbrace \smgs'' \rbrace) \subseteq \smsg
      \textrm{a query}~()~\textrm{was made to \SBCMSign}: \pcreturn 1 \\
      \pcreturn 0
    }
  \end{minipage}
  \label{fig:euf-game}
