...
\begin{figure}[ht!]
  \scalebox{0.9}{  
    \begin{minipage}[t]{\textwidth}
      \centering      
      \procedure{$\ExpINDCCAiib(\secpar)$}{%
        \parm \gets \ESetup(\secpar) \\
        (\ek,\dk) \gets \EKeyGen(\parm) \\
        b^* \gets \adv^{\oracle{\ELR}(b,\cdot,\cdot),
          \oracle{\EDEC}(\dk,\cdot)}(\ek),
        ~\textrm{where:} \\
        \pcind \oracle{\ELR}(b,\msg_0,\msg_1)~\textrm{returns}~\EEnc(\ek,\msg_b),
        ~\textrm{and} \\
        \pcind \oracle{\EDEC}(\dk,\Ec)~\textrm{returns}~\EDec(\dk,\Ec) \\
        \pcif \oracle{\EDEC}~\textrm{has been called with an output of \ELR,
          abort} \\
        \pcreturn b^*
      }
    \end{minipage}
  ...
  \scalebox{0.9}{
    \begin{minipage}[t]{0.5\textwidth}
      \procedure{$\ExpComBind(\secpar)$}{%
        \parm \gets \CSetup(\secpar) \\
        (\msg_0,r_0,\msg_1,r_1) \gets \adv(\parm) \\
        \Ccom_0 \gets \CCommit(\parm,\msg_0,r_0) \\
        \Ccom_1 \gets \CCommit(\parm,\msg_1,r_1) \\
    ...
    \begin{minipage}[t]{0.5\textwidth}
      \procedure{$\ExpComHideb(\secpar)$}{%
        \parm \gets \CSetup(\secpar) \\
        b' \gets \adv^{\oracle{COM}(\parm,\cdot,\cdot)},~\textrm{where}: \\
        \pcind COM(\parm,\msg_0,\msg_1): \\
        \pcind \pcind r \getr \mathcal{R};~\pcreturn \CCommit(\parm,\msg_b,r) \\
  ...
  \scalebox{0.85}{
    \begin{minipage}[t]{0.55\textwidth}
      ...
      \procedure{$\ExpNIZKSimExt(\NIZKsecpar)$}{%
        ...
        \pcreturn \Verify(\NIZKcrs,\NIZKproof,\NIZKx) = 1 \land
        (\NIZKx,\NIZKw') \notin \NIZKRel~\land \\
  ...
  \scalebox{0.85}{
    \begin{minipage}[t]{0.62\textwidth}
      ...
      \procedure[linenumbering]{$\ExpSBCMOMF(\secpar)$}{%
        ...
        \lbrace(\sig_i,\osmsg_i, \smsg_i)\rbrace_{i\in[n]} \gets
    ...
    \begin{minipage}[t]{0.43\textwidth}
      \procedure[linenumbering]{$\ExpSBCMBlindb(\secpar)$}{%
        ...
        b^* \gets \adv^{\oracle{BLIND}
          (\cdot,\cdot,\cdot)}(\vk),~\textrm{where:} \\
        \pcind \oracle{BLIND}(\osmsg_0,\osmsg_1,\smsg): \\
...
\oracle{BLIND} oracle used in the blindness game, so that besides the $\osmsg_0$
...
\notin \NIZKRel$, for $b=0,1$, \oracle{BLIND} aborts. Otherwise, the output
...
\paragraph{$\parm \gets \SBCMSetup(\secpar,\nattrs,\tnattrs)$.} %
...
% \paragraph{$\SBCMsig/\bot \gets \langle \SBCMCom(\SBCMvk,\osmsg,
...
\vk it receives. Then, for every query that \adv~makes to its \oracle{BLIND}
oracle, \advB checks that the length of \smsg is acceptable (i.e., less than the
maximum length allowed by \SBCM), and forwards the query to its own \oracle{COM}
oracle using $\osmsg_0$ and $\osmsg_1$. To the output \Ccom of \oracle{COM},
