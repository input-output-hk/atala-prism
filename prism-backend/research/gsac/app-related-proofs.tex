...
\begin{proof}
  ...
  Now, suppose that \adv~wins. Then, by assumption, the identity extracted by
  \adv~(which maps to a $\cid$ for \advB), does not belong to the set of corrupt
  users in \adv's game. By construction of \advB, then, $\OWNR[\cid] \notin \CU$
  in \advB's unforgeability game.
  \todo{The following seems wrong.}
  But it cannot belong to an honest user either,  
  as \adv's signature does not exist in \SIG. Otherwise, suppose it belongs to
  an honest user (in list \HU of \advB's game). Then, it breaks one-more
  unforgeability of the \SBCM scheme used by \CUASGen, as it is a valid.
  %
  \qed
\end{proof}
