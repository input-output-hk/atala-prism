...
\sig,\overline{\smsg},\smsg)$ verifies a signature \SBCMsig over message set
...
We use common notation habits. $y \gets A(x)$ means that algorithm $A$, on input
$x$, produces output $y$. When $A$ is a probabilistic process, and we want to
make explicit the randomness $r$, we use $y \gets A(x;r)$. $\langle y_A,y_B \gets
\langle A(x_A),B(x_B)\rangle$ denotes algorithms $A$ and $B$, running an
...
issuer public key, \sipk denotes a set of issuer's public keys.
%
