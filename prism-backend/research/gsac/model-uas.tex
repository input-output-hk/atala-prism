...
\todo{Do our definitions protect from absurd schemes that leak \usk or \scred via
  \fissue or \feval (I guess \finsp is ok)? E.g., $\feval(\usk,\scred,\msg) =
  \usk$ deanonymizes the signer. I guess that anonymity should prevent that.
  But check.}
...
\todo{Mention (probably, in future work?) that since the public keys of the
  credential issuers are assumed to be public, we must enforce that challenge
  users with same issuers per credential are used in the challenge oracle. This
  is to avoid trivial wins. But not sure if the naive extension of hiding also
  the issuers would make sense in the real world (which issuers are trusted?)}
