...
\item[$(\y,\iproof)/\bot \gets \Inspect(\gpk,\osk,\trans,\sig,\msg,\feval)$.] %
  \todo{I think we do not need \trans here, see the generic construction. (This
    was somehow inherited from \cite{bsz05}.) However, if we do remove it, we'd
    need to redefine the traceability game -- it'd need to depend on \Identify,
    which kind of makes sense too.}
...
\item[$\fissue: (\upk, \attrs,\scred) \rightarrow 0/1$.] Defined by issuers,
  ...
  additional credentials \scred, all bound to \upk, which may further be used
  for blind issuance \commentwho{Jesus}{can it?}. \fissue returns $1$ to accept
  a request, $0$ to reject it.  
\item[$\feval: (\upk,\cred,\msg) \rightarrow \varrngfeval$.] Can be defined by
