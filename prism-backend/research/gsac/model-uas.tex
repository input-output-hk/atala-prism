...
\paragraph{Helper Function \Identify.} %
\todo{We use extract now...} In addition, we make use of a helper
...
\begin{figure}[htp!]
  \procedure{$\ExpCorrect(1^\secpar)$}{%
    ...
    \pcif \CorrectEval(\oid,\uid,\scid,\msg,\sig,\yeval,\feval) = 0: \pcreturn 1~
    \pccomment{The result of evaluation is wrong} \\
    \pcif \CorrectInspect(\oid,\uid,\scid,\msg,\sig,\yeval,\feval) = 0: \pcreturn 1~
    \pccomment{The result of inspection is wrong} \\
    \pcreturn 0 \\
  }
  ...
  \procedure{$\CorrectEval(\oid,\uid,\scid,\msg,\sig,\yeval,\feval)$}{%
    \pcif \feval(\UK[\uid],\CRED[\scid],\msg) \neq \yeval: \pcreturn 0 \\
    \pcreturn 1 \\
  }

  \procedure{$\CorrectInspect(\oid,\uid,\scid,\msg,\sig,\yeval,\feval)$}{
    \textrm{Parse}~\OK[\oid]~\textrm{as}~((\cdot,\finsp^\gid),\cdot) \\
    \pccomment{\sig cannot be inspected} \\
    \pcif \Inspect(\PRVOK[\oid],\PUBIK[\scid],\sig,\yeval,\msg,\feval) = \bot:
    \pcreturn 0 \\
    \pccomment{The produced $(\y,\iproof)$ pair is rejected by \Judge} \\
    \pcif (\yinsp,\iproof) \gets \Inspect(\PRVOK[\gid],\PUBIK[\scid],\sig,
    \msg,\feval)~\land \\
    \hspace*{10pt}
    \Judge(\PUBOK[\oid],\PUBIK[\scid],\y,\iproof,\sig,\yeval,\msg,\feval) = 0: \\
    \pcind \pcreturn 0 \\
    \pccomment{\yinsp is the wrong value} \\
