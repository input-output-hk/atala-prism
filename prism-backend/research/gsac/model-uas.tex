    ...
    \scalebox{0.9}{
      ...
      \begin{minipage}[t]{0.55\textwidth}
        ...
        \procedure{$\CHALb(\oid,\cuid_{0,1},\cscid_{0,1},\msg,\feval)$}{%
          ...
          (\yeval,\cdot) \gets \feval(\UK[\cuid_0],\CRED[\cscid_0],\msg) \\
          ({\yeval}',\cdot) \gets \feval(\UK[\cuid_1],\CRED[\cscid_1],\msg) \\
          \pcif \yeval \neq {\yeval}': \pcreturn \bot \\
          ...
          \CSIG[\cSig_b] \gets 
          \lbrace (\oid,\cuid_b,\cscid_b,\msg,\feval,\\
...
\begin{figure}[htp!]
  \procedure[linenumbering]{$\ExpCorrect(1^\secpar)$}{%
    ...
    \pcif \OWNR[\scid] \neq \uid: \pcreturn 0 \\
    (\Sig = (\sig,\yeval,\ceval)) \gets \Sign(\UK[\uid],\PUBOK[\oid],\scid,\msg,
    \feval) \\
...
of the anonymity game is given in \figref{fig:exp-uas-anonb}, where $\Oanon
...
\begin{figure}[htp!]
    \procedure[linenumbering]{$\ExpForgeSign(1^\secpar)$}{%
      \parm \gets \Setup(1^\secpar) \\
      (\oid,\siid,\Sig=(\sig,\yeval,\ceval),\msg,\feval) \gets \adv^{\Oforgesign}(\parm) \\
      \pcif \exists \uid~\st~(\cdot,\cdot,\Sig,\msg,\feval) \in
      \SIG[\uid]: \pcreturn 0 \\
...
\begin{figure}[htp!]
  \procedure[linenumbering]{$\ExpNonframe(1^\secpar)$ \todo{Updated win conditions here. Update proof.}}{%
    \parm \gets \Setup(1^\secpar) \\
    (\oid,\siid,\Sig=(\sig,\yeval,\ceval),\msg,\feval,\yinsp,\iproof) \gets
    \adv^{\Oframe}(\parm) \\
...
%    \pcreturn 0 \\
    ...
    \pcif \nexists (\cdot,\cdot,\Sig,\msg,\feval) \in \SIG[\uid]: \pcreturn 1 \\
    \pcif \finsp((\yeval^0,\yeval^1),\usk,\scred,\msg) \neq \yinsp \lor
