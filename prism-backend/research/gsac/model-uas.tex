...
\item[$1/0 \gets \Verify(\opk, \lbrace \ipk_i \rbrace_{i \in \Issuers},\sig,
  ...
  keys $\ipk_i$, for evaluation function \feval and opener key \opk.
...
\paragraph{Helper functions \ExtractIssue, \ExtractSign, \IdentifyCred, and
  \IdentifyUK.} In our modelling, we assume the existence of four functions,
\ExtractIssue, \ExtractSign, \IdentifyCred, and \IdentifyUK. They are not
...
\item[$\IdentifyUK(\uid,\usk)$.] Returns $1$ is \usk is {\uid}'s secret key.
  This is trivial for honest users, for which this must be a one-to-one
...
the concrete construction. \jdv{Although, for the case of \ExtractIssue,
  online extractability (or alternative requirements, such as non-parallel or
  logarithmic number of joins) is necessary.}
    ...
    \scalebox{0.9}{
      ...
      \begin{minipage}[t]{.5\textwidth}
        ...
        \procedure{$\OPEN(\oid,\sig,\yeval,\msg)$}{%
          ...
          \in \SIG[\uid] \\
          (\yinsp,\iproof) \gets
          \Open(\PRVOK[\oid],\GK[\scid],\sig,\yeval,\msg,\feval) \\
          \pcif \CSIG[\sig] \neq \bot: \\
...
It is a quite strong property, as it must be ensured even in the presence of
dishonest issuer and opener. Intuitively, it prevents the adversary from
creating a signature that can be (even in some generic sense) associated to an
honest user who did not produce it. Depending on the opening capabilities of the
scheme, this framing could be done in different ways; i.e., by convincing third
parties that signatures by different (possibly corrupt) users are linked, or
directly by having opening proofs output the identity of a user who did not
create the signature being opened.
