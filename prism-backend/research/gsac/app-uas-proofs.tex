...
\begin{proof}[\thmref{thm:forge-uas}; Signature unforgeability of \CUASGen]
  ...
  \paragraph{(e) All {\cred}s are bound to the same \usk.} %
  ...
  Assume that no such \uid exists. Such a signature breaks the one-more
  forgery property of \SBCM, as it has not been produced via a call to \SIGN
  (it is not in $\SIG$). Therefore, the probability that \adv~wins at line 14 is
  negligible, given simulation extractability of $\NIZK^{\Sign}$, and
  correctness and one-more forgery security of \SBCM.
  %
  \qed
\end{proof}
...
\begin{proof}[\thmref{thm:frame-uas}; Non-frameability of \CUASGen]
  ...
  First, suppose that the \Sig that \adv~outputs at line 2 exists in \SIG, for
  some \uid. In that case, the signature is accepted by \Verify at
  ...
  $\NIZKRel_{\Sign}$, and blindness of \SBCM, $\Sig \in \SIG$ except with
