...
\begin{proof}[\thmref{thm:issue-anonymity-uas}; Issuance anonymity of \CUASGen]
  ...
  \HUGEN, and assigning to the $q$-th user the \vk key that \advB receives in
  its blindness game. Then, when \adv~makes a query to its \OBTCHALb oracle, if
  the chosen issuer is the $q$-th user, \advB forwards \adv's messages to its
  own \oracle{BLIND} oracle. Finally, \advB outputs \todo{XXXX UPDATE}
  whatever \adv~does. Note that, since in Game 2 we replace the \yissue value
  with a random value, and simulate the \RelIss proof, \adv~is not
