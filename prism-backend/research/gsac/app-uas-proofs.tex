...
\begin{figure}[ht!]
  ...
  \scalebox{0.9}{
    \procedure{$\Exp^{\UAS-acc-b}_\adv(1^\secpar)$}{%
      ...
      \pcfor d\in\bin: \\
      \pcind (\upk_d,\usk_d) \gets \KeyGen(\parm) \\
      \pcind (\st,\attrs, \lbrace (\cidi_d,\cidu_d,\cattrs_{d,i}) \rbrace_{i\in[n]})
      \gets \adv(\st,\upk_0,\upk_1) \\
      \yissue^* \gets \fissue(\upk_b,
      \lbrace (\cidi_b,\cattrs_{b,i}) \rbrace_{i\in[n]},
      \attrs) \\
...
\begin{proof}[\thmref{thm:issue-anonymity-uas}; Issuance anonymity of \CUASGen]
  ...
  First, observe that, excluding the \yissue value output revealed to the issuer
  during the interactive issuance protocol, \CUASGen essentially runs an
  interactive signing protocol of an \SBCM scheme, augmented with a
  zero-knowledge NIZK for $\NIZKRel_{\Issue}$. As stated in \appref{sapp:sbcm},
  blindness of the (non-augmented) \SBCM scheme, and the zero-knowledge property
  of the NIZK for $\NIZKRel_{\Issue}$, ensure that the challenge credentials are
  indistinguishable between thesmselves. Concretely, we can replace the NIZK
  proofs sent by the user by simulated proofs.

  Finally, note that, to avoid trivial wins by the adversary, the issuance
  anonymity game requires that \fissue either rejects (returns $\yissue=0$) for
  both challenge users, or it accepts for both (returns $\yissue \neq 0$).
  Moreover, in the latter case, since we require that \fissue functions are
  \UAS-acceptable functions, the resulting \yissue value is hiding with respect
  to the input arguments. Thus, adding \yissue does not allow the adversary to
  distinguish credential issuance protocols.  
\end{proof}
