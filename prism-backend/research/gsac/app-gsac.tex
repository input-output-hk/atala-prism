...
\todo{Need to update the proofs with the updated definitions, now returning
  \attrs as part of \Open.}

\commentwho{Jesus}{Strictly, in our generic construction we require IND-CCA, but
  in the following concrete instantiation, we use ElGamal, which is IND-CPA.
  However, we actually ``lift'' ElGamal with NIZKs of the encrypted plaintexts,
  which is essentially IND-CCA as far as I know. Still, make sure to mention
  that.}

\begin{proof}[\thmref{thm:correctness-gsac}; Correctness of \GSACGen]
  \todo{XXX}
\end{proof}

\begin{proof}[\thmref{thm:anonymity-gsac}; Anonymity of \GSACGen]
...
\begin{proof}[\thmref{thm:trace-gsac}; Traceability of \GSACGen]
...
\begin{proof}[\thmref{thm:frame-gsac}; Non-frameability of \GSACGen]
  Consider the following events:
