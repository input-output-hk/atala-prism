  ...
  \scalebox{0.9}{      
    \begin{minipage}[t]{0.5\textwidth}        
      \procedure[linenumbering]{$\Exp^{anon-b}_{\adv,ac}(1^\secpar)$}{%
        \parm \gets \Setup(1^\secpar) \\
        b^* \gets \adv^{HU,CU,Obtain,Show,LoR}(\parm) \\
        \pcreturn b^*
      }        
    \end{minipage}
  }
...
\todo{Rewrite when the full rewriting of the paper is done. For now, just
  adapting the construction.}
We give a simple definition for delegatable anonymous credentials extending
the anonymous credential model in \cite{fhs19}, but drawing inspiration from
\cite{bcc+09}. Next, we briefly describe a construction based on $(\fissue^n,
\feval^{n,\dattrs},\finsp^0)$-\CUASGen, and
...
\begin{figure}[ht!]
  ...
  \scalebox{0.9}{      
    \begin{minipage}[t]{.6\textwidth}
      \procedure[linenumbering]{$\Exp^{forge}_{\adv,dac}(1^\secpar)$}{%
        \parm \gets \Setup(1^\secpar) \\        
        (D,n+1,\st) \gets \adv^{\mathcal{O}_{forge}}(\parm,\opk) \\
  ...
  \caption{Security games for delegatable anonymous credentials with selective
    ...
    CU,Obtain,Issue,Show,IssLoR\rbrace$, $\mathcal{O}_{sig-anon} \gets \lbrace
    HU,CU,Obtain,Issue,Show,SigLoR\rbrace$. $\mathcal{O}_{forge} \gets \lbrace
    HU,CU,ObtIss,Issue,Show \rbrace$. $IssLor$ is equivalent to \OBTCHALb in
    \UAS, whereas $SigLor$ is equivalent to \CHALb. Also, \OWNR and \ATTR
    tables are essentially as in \secref{ssec:model-uas}.
    $level(\opk,n)$ is a helper function that returns the $1$ if the level of
    \opk is $n$, or $0$ otherwise.}
  \label{fig:model-dac}  
\end{figure}

\subsubsection{\CUASDAC Construction.} %
In a nutshell, we distinguish between top-level issuers, and users who delegate
their credentials, thus acting as lower-level issuers. Top-level issuers are
exactly as issuers in \CUASGen. Lower-level issuers need to make public an
issuance public key that reveals the attributes they are willing to delegate,
and their credential level. The issuance function has to prove that the
requestor is asking
for a credential that: (1) increments the credential level exactly by one, and
(2) the requested attributes are a subset of those that the low-level issuer
can delegate. Note that both conditions apply recursively. To prevent
mix-and-match attacks, in each (lower-level) credential we include as attributes
a ``serial number'' of the immediately upper level credential, and another one
for the credential being issued (\todo{see figure}). For a credential of
level $n$, the evaluation function must output a chain of proofs of knowledge of
a sequence of credentials of levels $n$ up to $1$, with matching serial number
pairs.

More concretely, we define issuance functions for low-level issuers to have
the following structure:

\begin{align}
  & \fissue^{n,r_n,\attrs}(\usk,\scred,\attrs') \coloneqq \lbrace
    \pcif \attrs'_1 = n+1 \land \attrs'_2 = r_n \land
    \attrs'_{[4..t]} \subseteq \attrs: \pcreturn 1; \pcelse \pcreturn 0
    \rbrace \nonumber \\
  &  \feval^{n,\dattrs}(\usk,\cred,\msg) \coloneqq \lbrace \textrm{Parse \cred
    as }(\cred',\Sig,r); \pi \gets K(\cred') \rbrace
\end{align}
\todo{XXXX}

Where $n$ is its depth in the delegation chain, $\attrs'_3$ must be some serial
number chosen by the issuer for the new credential, and $r_n$ is a random number
serving as serial number for the issuer's credential (chosen by its parent
issuer), and \attrs is the attribute set that it can delegate. Note that this
issuance funcion expects the first attribute of the requested credential to
encode the credential depth, the second attribute to encode the serial number
of the issuer's credential, and the third attribute to encode the random
number (chosen uniformly by the issuer) that will serve as identifier of the
produced credential. All other attributes only need to meet the condition of
being contained in the attributes \attrs of the issuer's credential. 

\begin{description}
\item[$\parm \gets \Setup(1^\secpar)$.] As in \CUASAC.
\item[$(\ipk,\isk) \gets IssKeyGen(\parm)$.] If the issuer is a top-level
  issuer, $IssKeyGen$ simply runs \IKeyGen in \CUASAC. If it is a lower-level
  issuer (say, of level $n>1$), this is in fact a user, who has already run
  $UserKeyGen$ below (and thus, \IKeyGen), and owns a credential  over
  attributes \attrs, issued by some higher-level issuer. Thus, instead of
  running \SBCMKeyGen, sets $\isk \gets \usk$, and its issuance function to
  $\fissue^{n,r_n,\attrs}$. % The
  % lower-level issuer also runs $(\sig,\yeval) \gets \Sign(\usk,\opk,\cred,r,
  % \feval^{\dattrs})$, where $\feval^{\dattrs}$ is as defined as in
  % \appref{sapp:related-models-ac}, and thus reveals the attributes that the
  % low-level issuer can delegate. That is, by definition of $\feval^{\dattrs}$,
  % given the \attrs defined in $\fissue^{n,r_n,\attrs}$, $\yeval = \attrs$ (also,
  % note that the issuer's credential may contain other non-delegatable
  % attributes).
  Finally, the lower-level issuer sets $\ipk \gets (\ipk,n,r_n,\attrs,
  \fissue^{n,r_n,\attrs})$.
...
\item[$\langle \cred/\bot,\top/\bot \rangle \gets
  ...
  We assume that the set \attrs of attributes to be included in the credential
  are of the form $(n+1,r_n,r_{n+1},\attrs')$, where $n+1$ is the level of the
  credential to be issued, $r_n$ is the identifier of the issuer's credential
  (or some special fixed value if it is a top level credential), $r_{n+1}$ is
  a serial number for the new credential, chosen by the issuer, and $\attrs'$
  is the attributes being requested by the user. Both issuer and user agree on
  these values beforehand. If the issuer is top-level, this protocol runs as in
  \CUASAC, for \attrs as above. Else, if the issuer is not top-level (say, it
  has level $n>1$), the protocol in \CUASAC is augmented as follows:

  \begin{itemize}
  \item \uline{User}: $r \gets \bin^l$ (for some appropriate $l$).
  \item \uline{User,Issuer}: Run $\langle\Obtain,\Issue\rangle$ as in \CUASGen,
    for \attrs as specified above.
  \item \uline{Issuer}: $\Sig=(\sig,\yeval) \gets \Sign(\usk,\opk,\cred,r,
    \feval^{n,\dattrs})$. Where \cred is the issuer's $n$-th level credential,
    and $\feval^{n,\dattrs}$ is as in \appref{sapp:related-models-ac}, plus
    revealing the credential level (encoded in \cred's first attribute).
  \item \uline{User}: Accept if $\Verify(\opk,\ipk,\Sig,r,\feval^{n,\dattrs})=
    1$, and return $(\cred,\ipk,\Sig,r)$ as $(n+1)$-level credential, where
    \cred is the credential produced in the interactive protocol at step 2.
    Otherwise, reject.
  \end{itemize}

  Note that, basically, the issuer proves that it owns a credential of level $n$
  over the attributes it is delegating.

\item[$1/0 \gets \langle Show(\opk,\attrs,\dattrs,\cred),
  \Verify(\opk,\dattrs) \rangle$.]
  As in \CUASAC, but using $\feval^{n,\dattrs}$ as signature evaluation
  function.
\end{description}
