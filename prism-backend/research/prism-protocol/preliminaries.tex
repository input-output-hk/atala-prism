...
\begin{figure}[ht]
  \begin{framed}
    ...
    \procedure{$\pcif \cmd = \uccmd{HashQuery} \land \arg = \msg$}{%
      ...
      \ucio{Output}~(\uccmd{HashConfirm},\sid,h)
    }
  \end{framed}
\end{figure}
  ...
  \begin{framed}
    ...
    \begin{minipage}[t]{0.60\textwidth}
      \procedure{$\pcif \cmd = \uccmd{Deregister} \land \arg = \emptyset$}{
        ...
        \text{Output}~(\uccmd{DeregisteredOk},\sid)~\text{to}~P \\
      }
      \procedure{$\pcif \cmd = \uccmd{Read} \land \arg = \emptyset$}{
        \text{Output}~(\uccmd{ReadOk},\sid,t)~\text{to}~P
      }
    \end{minipage}
  \end{framed}
  ...
  \begin{framed}
    ...
    \begin{minipage}[t]{0.40\textwidth}
      \procedure{$\pcif \cmd = \uccmd{Submit} \land \arg = \ldgTx$}{
        \ucio{Send}~(\uccmd{Read},\sid)~\text{to}~\IdealGclock \\
        \ucio{Recv}~(\uccmd{ReadOk},\sid,t)~\text{from}~\IdealGclock \\
        \ldgUtxo \gets \ldgUtxo \cup \set{(\ldgTx,t)} \\ 
        \ucio{Output}~(\uccmd{Submitted},\sid,\ldgTx,t)~\text{to}~\adv \\
      }
    ...
    \begin{minipage}[t]{0.60\textwidth}
      \procedure{$\pcif \cmd = \uccmd{Read} \land \arg =
        ...
        \pcreturn \bot \\        
        \text{Output}~(\uccmd{ReadOk},\sid,\ldgHead(\ldgMap(P)))~\text{to}~P \\
      }
