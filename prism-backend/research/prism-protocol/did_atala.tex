  ...
  \begin{framed}
    ...
    \begin{minipage}[t]{0.5\textwidth}
      ...
      \procedure[linenumbering]{$\pcif \cmd = \uccmd{Verify} \land
        ...
        \ucsend~(\uccmd{Verify},\sid,\msg,\sig,v)~\text{to}~\adv \\
        \ucrecv~(\uccmd{Verified},\sid,\msg,\phi)~\text{from}~\adv \\
    ...
    \scalebox{0.9}{
      \begin{minipage}[t]{0.55\textwidth}
        \procedure[linenumbering]{$\pcif \cmd = \uccmd{Create} \land \arg =
          ...
          \pcind \text{from}~\adv \\
          \pcif (P,\did,\cdot) \notin \DID \land
          \P(\lbrace (i, \lbl_i) \rbrace_{i\in[n]}) = 1: \\
          \pcind \DID \gets \DID \cup \lbrace (P,did,\lbrace
          (i,\lbl_i,\val_i) \rbrace_{i\in[n]}) \rbrace \\
    ...
    \scalebox{0.9}{
      \begin{minipage}[t]{0.55\textwidth}
        \procedure[linenumbering=on]{$\pcif \cmd = \uccmd{Create} \land \arg =
          ...
          \ucsend~(\uccmd{Sign},(P,\sid,j),\tx)~\text{to}~\IdealFSig \\
          \ucrecv~(\uccmd{Sig.},(P,\sid,j),\tx,\sig)~\text{from}~\IdealFSig \\
    ...
    \scalebox{0.9}{
      \begin{minipage}[t]{0.5\textwidth}
        \procedure[linenumbering=on]{$\pcif \cmd = \uccmd{Read} \land
          ...
          \pcind \ucsend~(\uccmd{Verify},(P,\sid),\tx,\sig,\pk)~\text{to}~\IdealFSig \\
          \pcind \ucrecv~(\uccmd{Verified},(P,\sid),\tx,b)~\text{from}~\IdealFSig \\
    ...
    \scalebox{0.90}{          
      \begin{minipage}[t]{0.55\textwidth}
        \procedure[linenumbering=on]{$\pcif \cmd = \uccmd{Update} \land
          ...
          \pcfor i \in \tilde{N}~\suchthat~ i > n : \\
          \pcind \ucsend~(\uccmd{KeyGen},(P,\sid,i))~\text{to}~\IdealFSig \\
          \pcind \ucrecv~(\uccmd{VerKey},(P,\sid,i),\pk_i)~\text{from}~
          \IdealFSig \\
