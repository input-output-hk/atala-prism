...
\commentwho{Jesus}{What follows is essentially right on its own, but I interpreted
  \cite{bhz21} somehow wrongly. The approach is: (1) merge all (ideal) global
  subroutines (ledger, RO, clock) into one (\gAtala); (2) prove that theorem 1
  below is met, where the real protocol is \RealPKIDIDAtala, and the ideal
  (global) functionality is \IdealGPKIDID -- although we can ignore that it is
  a global functionality here. Then, it would be nice to prove that Cardano as
  instantiation of the ledger, and whatever else for RO and clock, allow to
  instantiate the merged global subroutine (this is what I've mostly done next,
  to some extent). So, basically, revert the order of what I was trying to do.}
