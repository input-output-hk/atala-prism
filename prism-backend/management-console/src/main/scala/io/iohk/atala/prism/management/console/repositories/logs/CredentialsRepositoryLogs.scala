...
private[repositories] final class CredentialsRepositoryLogs[F[_]: MonadThrow: ServiceLogging[*[
  _
  ...
  override def create(
    ...
    in =>
      info"creating credentials $participantId" *> in
        ...
        .onError(errorCause"encountered an error while creating credentials" (_))
  ...
  override def getBy(credentialId: GenericCredential.Id): Mid[F, Option[GenericCredential]] =
    in =>
      info"getting credential by credential id $credentialId" *> in
        .flatTap(r => info"getting credential by credential id result - ${r.fold("not found")(_ => "found")}")
        .onError(errorCause"encountered an error while getting credential by credential id" (_))
  ...
  override def getBy(issuedBy: ParticipantId, query: PaginatedQuery): Mid[F, List[GenericCredential]] =
    in =>
      info"getting credentials by query" *> in
        ...
        .onError(errorCause"encountered an error while getting credentials by query" (_))
  ...
  override def getBy(
    ...
    in =>
      info"getting credentials by maybe last seen credential id $lastSeenCredential" *> in
        ...
        .onError(errorCause"encountered an error while getting credentials by maybe last seen credential id" (_))
  ...
  override def getBy(issuedBy: ParticipantId, contactId: Contact.Id): Mid[F, List[GenericCredential]] =
    in =>
      info"getting credentials by contact id $contactId $issuedBy" *> in
        ...
        .onError(errorCause"encountered an error while getting credentials by contact id" (_))
    ...
    in =>
      info"storing publication data $issuerId" *>
        in.flatTap(_ => info"storing publication data - successfully done")
          .onError(errorCause"encountered an error while storing publication data" (_))
    ...
    in =>
      info"storing batch data $batchId" *>
        in.flatTap(_ => info"storing batch data - successfully done")
          .onError(errorCause"encountered an error while storing batch data" (_))
    ...
    in =>
      info"storing revocation data $institutionId $credentialId" *>
        in.flatTap(_ => info"storing revocation data - successfully done")
          .onError(errorCause"encountered an error while storing revocation data" (_))
}
