...
object ConnectorClient {
  ...
  private final class GrpcImpl[F[_]](
    ...
    override def generateConnectionTokens(
      ...
      val newStub = connectorService.withInterceptors(
    ...
    override def getConnectionStatus(
      ...
      val metadata = header.toMetadata
      val newStub = contactConnectionService.withInterceptors(
        MetadataUtils.newAttachHeadersInterceptor(metadata)
      )
...
private[clients] final class ConnectorClientMetrics[F[_]: TimeMeasureMetric: MonadCancel[*[_], Throwable]]
  ...
  val sendMessagesTimer: DomainTimer =
    TimeMeasureUtil.createClientRequestTimer(clientName, "sendMessages")
  ...
  override def generateConnectionTokens(
    ...
    _.measureOperationTime(generateConnectionTokensTimer)
