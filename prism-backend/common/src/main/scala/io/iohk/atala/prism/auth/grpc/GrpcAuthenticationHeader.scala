...
object GrpcAuthenticationHeader {
  ...
  final case class PublishedDIDBased(requestNonce: RequestNonce, did: PrismDid, keyId: String, signature: ECSignature)
      extends DIDBased
  final case class UnpublishedDIDBased(requestNonce: RequestNonce, did: PrismDid, keyId: String, signature: ECSignature)
      extends DIDBased
}
