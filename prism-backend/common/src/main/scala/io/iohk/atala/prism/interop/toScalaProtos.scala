...
object toScalaProtos {
  ...
  implicit class CreateDIDOperationInterop(private val v: io.iohk.atala.prism.protos.CreateDIDOperation.DIDCreationData)
      extends AnyVal {
    def asScala: node_models.CreateDIDOperation.DIDCreationData = {
      node_models.CreateDIDOperation.DIDCreationData.parseFrom(MessageKt.encodeToByteArray(v))
    }
  }
