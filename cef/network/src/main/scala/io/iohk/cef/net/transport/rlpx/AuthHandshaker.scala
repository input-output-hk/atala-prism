...
import io.iohk.cef.net.transport.rlpx.ethereum.crypto.ECIESCoder
...
import io.iohk.cef.net.transport.rlpx.ByteUtils._
...
case class AuthHandshaker(
  ...
  def initiate(uri: URI): (ByteString, AuthHandshaker) = {
    ...
    val message: AuthInitiateMessageV4 = createAuthInitiateMessageV4(remotePubKey)
  ...
  def handleResponseMessageV4(data: ByteString): AuthHandshakeResult = {
    ...
    val message = rlp.decodeFromArray[AuthResponseMessageV4](plaintext)
