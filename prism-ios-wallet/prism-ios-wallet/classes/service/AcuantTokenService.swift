...
public class AcuantTokenService: IAcuantTokenService{
    ...
    public func getTask(callback: @escaping (String?) -> ()) -> URLSessionTask?{
        ...
        if  let endpoint = AcuantCommon.Credential.endpoints()?.acasEndpoint,
            let urlPath = NSURL(string: "\(endpoint)/oauth/token"),
            let username = AcuantCommon.Credential.username(),
            let password = AcuantCommon.Credential.password()
        {
            let request = NSMutableURLRequest(url: urlPath as URL)
            
            let sub = (AcuantCommon.Credential.subscription() != nil && !AcuantCommon.Credential.subscription()!.isEmpty) ? "\(AcuantCommon.Credential.subscription()!);" : ""
