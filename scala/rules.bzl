load("@io_bazel_rules_scala//scala:scala.bzl", native_scala_library = "scala_library", native_scala_binary = "scala_binary", native_scala_test = "scala_test", "scala_repl")
...
def scala_test(name, srcs=None, deps=[], scalac_jvm_flags = _default_scalac_jvm_flags,  **kwargs):
  native_scala_test(
    name = name,
    deps = deps,
    srcs = native.glob(["test/**/*"]) if srcs == None else srcs,
    scalac_jvm_flags = scalac_jvm_flags,
    **kwargs
  )
#  scala_repl(
