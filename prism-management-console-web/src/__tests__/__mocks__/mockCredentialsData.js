export const contacts = [
  {
    externalid: '501619c5-270a-4e80-9e70-f0cf9bcef5e5',
    contactName: 'contact name'
  },
  {
    externalid: '14c17a25-30a9-4e3d-a63d-83ebd8a2dd59',
    contactName: 'contact name'
  },
  {
    externalid: '614e9d7a-71c8-4a31-a1d2-8c1928c0541d',
    contactName: 'contact name'
  },
  {
    externalid: 'd920875f-ddec-4c0b-ab1c-5c2b9c8ee415',
    contactName: 'contact name'
  },
  {
    externalid: 'a84b5652-99d4-41d4-9595-ce4d349baa90',
    contactName: 'contact name'
  },
  {
    externalid: '1708bfa5-4c8c-4c0c-b358-06fd088c0a89',
    contactName: 'contact name'
  },
  {
    externalid: '0718e98c-56d8-404d-9f14-37cf3c6c2f77',
    contactName: 'contact name'
  },
  {
    externalid: '03ada254-dbcf-4fa1-a914-fa6585fb50a2',
    contactName: 'contact name'
  },
  {
    externalid: '976396d0-d812-4f66-8117-491e0536a439',
    contactName: 'contact name'
  },
  {
    externalid: '4291fbfb-2f01-460e-8909-15e2d1db9dfc',
    contactName: 'contact name'
  },
  {
    externalid: 'a96e5db3-cd10-4571-9690-00a0e7caa664',
    contactName: 'contact name'
  },
  {
    externalid: '5fdcda7e-d492-4371-b028-c5fe67455762',
    contactName: 'contact name'
  },
  {
    externalid: '6a7a4484-fb43-43f6-8978-7d1197b254e3',
    contactName: 'contact name'
  },
  {
    externalid: '9565ba89-da3f-4cbe-9218-ab38f5c8bb30',
    contactName: 'contact name'
  },
  {
    externalid: '3676ad24-66a4-44e6-82eb-df9a456c3538',
    contactName: 'contact name'
  },
  {
    externalid: '7bdde45f-8840-4fc2-b43e-471db1e1748f',
    contactName: 'contact name'
  },
  {
    externalid: 'd3795d9e-32cf-48f6-8a4b-14517049c172',
    contactName: 'contact name'
  },
  {
    externalid: '1527aad2-c4af-4ca8-8c62-4b6e82f77609',
    contactName: 'contact name'
  },
  {
    externalid: '682373fc-f13b-4e39-9b3e-e46c5c234499',
    contactName: 'contact name'
  },
  {
    externalid: 'a96953c3-5ddc-4c61-bdb2-f4bd87d1d58e',
    contactName: 'contact name'
  }
];

export const validCredentials = {
  inputAoA: [
    [
      'External ID',
      'Contact Name',
      'ID Number',
      'Full Name',
      'Date Of Birth',
      'Expiration Date',
      'Photo'
    ],
    [
      '501619c5-270a-4e80-9e70-f0cf9bcef5e5',
      'contact name',
      '654654',
      'full name',
      '12/08/91',
      '17/12/49',
      ''
    ],
    [
      '14c17a25-30a9-4e3d-a63d-83ebd8a2dd59',
      'contact name',
      '661200',
      'full name',
      '12/08/91',
      '17/12/49',
      ''
    ],
    [
      '614e9d7a-71c8-4a31-a1d2-8c1928c0541d',
      'contact name',
      '667746',
      'full name',
      '12/08/91',
      '17/12/49',
      ''
    ],
    [
      'd920875f-ddec-4c0b-ab1c-5c2b9c8ee415',
      'contact name',
      '674292',
      'full name',
      '12/08/91',
      '17/12/49',
      ''
    ],
    [
      'a84b5652-99d4-41d4-9595-ce4d349baa90',
      'contact name',
      '680838',
      'full name',
      '12/08/91',
      '17/12/49',
      ''
    ],
    [
      '1708bfa5-4c8c-4c0c-b358-06fd088c0a89',
      'contact name',
      '687384',
      'full name',
      '12/08/91',
      '17/12/49',
      ''
    ],
    [
      '0718e98c-56d8-404d-9f14-37cf3c6c2f77',
      'contact name',
      '693930',
      'full name',
      '12/08/91',
      '17/12/49',
      ''
    ],
    [
      '03ada254-dbcf-4fa1-a914-fa6585fb50a2',
      'contact name',
      '700476',
      'full name',
      '12/08/91',
      '17/12/49',
      ''
    ],
    [
      '976396d0-d812-4f66-8117-491e0536a439',
      'contact name',
      '707022',
      'full name',
      '12/08/91',
      '17/12/49',
      ''
    ],
    [
      '4291fbfb-2f01-460e-8909-15e2d1db9dfc',
      'contact name',
      '713568',
      'full name',
      '12/08/91',
      '17/12/49',
      ''
    ],
    [
      'a96e5db3-cd10-4571-9690-00a0e7caa664',
      'contact name',
      '720114',
      'full name',
      '12/08/91',
      '17/12/49',
      ''
    ],
    [
      '5fdcda7e-d492-4371-b028-c5fe67455762',
      'contact name',
      '726660',
      'full name',
      '12/08/91',
      '17/12/49',
      ''
    ],
    [
      '6a7a4484-fb43-43f6-8978-7d1197b254e3',
      'contact name',
      '733206',
      'full name',
      '12/08/91',
      '17/12/49',
      ''
    ],
    [
      '9565ba89-da3f-4cbe-9218-ab38f5c8bb30',
      'contact name',
      '739752',
      'full name',
      '12/08/91',
      '17/12/49',
      ''
    ],
    [
      '3676ad24-66a4-44e6-82eb-df9a456c3538',
      'contact name',
      '746298',
      'full name',
      '12/08/91',
      '17/12/49',
      ''
    ],
    [
      '7bdde45f-8840-4fc2-b43e-471db1e1748f',
      'contact name',
      '752844',
      'full name',
      '12/08/91',
      '17/12/49',
      ''
    ],
    [
      'd3795d9e-32cf-48f6-8a4b-14517049c172',
      'contact name',
      '759390',
      'full name',
      '12/08/91',
      '17/12/49',
      ''
    ],
    [
      '1527aad2-c4af-4ca8-8c62-4b6e82f77609',
      'contact name',
      '765936',
      'full name',
      '12/08/91',
      '17/12/49',
      ''
    ],
    [
      '682373fc-f13b-4e39-9b3e-e46c5c234499',
      'contact name',
      '772482',
      'full name',
      '12/08/91',
      '17/12/49',
      ''
    ],
    [
      'a96953c3-5ddc-4c61-bdb2-f4bd87d1d58e',
      'contact name',
      '779028',
      'full name',
      '12/08/91',
      '17/12/49',
      ''
    ]
  ],
  expectedParse: [
    {
      externalid: '501619c5-270a-4e80-9e70-f0cf9bcef5e5',
      contactName: 'contact name',
      fullname: 'full name',
      idNumber: '654654',
      dateOfBirth: '12/08/91',
      expirationDate: '17/12/49',
      photo: ''
    },
    {
      externalid: '14c17a25-30a9-4e3d-a63d-83ebd8a2dd59',
      contactName: 'contact name',
      fullname: 'full name',
      idNumber: '661200',
      dateOfBirth: '12/08/91',
      expirationDate: '17/12/49',
      photo: ''
    },
    {
      externalid: '614e9d7a-71c8-4a31-a1d2-8c1928c0541d',
      contactName: 'contact name',
      fullname: 'full name',
      idNumber: '667746',
      dateOfBirth: '12/08/91',
      expirationDate: '17/12/49',
      photo: ''
    },
    {
      externalid: 'd920875f-ddec-4c0b-ab1c-5c2b9c8ee415',
      contactName: 'contact name',
      fullname: 'full name',
      idNumber: '674292',
      dateOfBirth: '12/08/91',
      expirationDate: '17/12/49',
      photo: ''
    },
    {
      externalid: 'a84b5652-99d4-41d4-9595-ce4d349baa90',
      contactName: 'contact name',
      fullname: 'full name',
      idNumber: '680838',
      dateOfBirth: '12/08/91',
      expirationDate: '17/12/49',
      photo: ''
    },
    {
      externalid: '1708bfa5-4c8c-4c0c-b358-06fd088c0a89',
      contactName: 'contact name',
      fullname: 'full name',
      idNumber: '687384',
      dateOfBirth: '12/08/91',
      expirationDate: '17/12/49',
      photo: ''
    },
    {
      externalid: '0718e98c-56d8-404d-9f14-37cf3c6c2f77',
      contactName: 'contact name',
      fullname: 'full name',
      idNumber: '693930',
      dateOfBirth: '12/08/91',
      expirationDate: '17/12/49',
      photo: ''
    },
    {
      externalid: '03ada254-dbcf-4fa1-a914-fa6585fb50a2',
      contactName: 'contact name',
      fullname: 'full name',
      idNumber: '700476',
      dateOfBirth: '12/08/91',
      expirationDate: '17/12/49',
      photo: ''
    },
    {
      externalid: '976396d0-d812-4f66-8117-491e0536a439',
      contactName: 'contact name',
      fullname: 'full name',
      idNumber: '707022',
      dateOfBirth: '12/08/91',
      expirationDate: '17/12/49',
      photo: ''
    },
    {
      externalid: '4291fbfb-2f01-460e-8909-15e2d1db9dfc',
      contactName: 'contact name',
      fullname: 'full name',
      idNumber: '713568',
      dateOfBirth: '12/08/91',
      expirationDate: '17/12/49',
      photo: ''
    },
    {
      externalid: 'a96e5db3-cd10-4571-9690-00a0e7caa664',
      contactName: 'contact name',
      fullname: 'full name',
      idNumber: '720114',
      dateOfBirth: '12/08/91',
      expirationDate: '17/12/49',
      photo: ''
    },
    {
      externalid: '5fdcda7e-d492-4371-b028-c5fe67455762',
      contactName: 'contact name',
      fullname: 'full name',
      idNumber: '726660',
      dateOfBirth: '12/08/91',
      expirationDate: '17/12/49',
      photo: ''
    },
    {
      externalid: '6a7a4484-fb43-43f6-8978-7d1197b254e3',
      contactName: 'contact name',
      fullname: 'full name',
      idNumber: '733206',
      dateOfBirth: '12/08/91',
      expirationDate: '17/12/49',
      photo: ''
    },
    {
      externalid: '9565ba89-da3f-4cbe-9218-ab38f5c8bb30',
      contactName: 'contact name',
      fullname: 'full name',
      idNumber: '739752',
      dateOfBirth: '12/08/91',
      expirationDate: '17/12/49',
      photo: ''
    },
    {
      externalid: '3676ad24-66a4-44e6-82eb-df9a456c3538',
      contactName: 'contact name',
      fullname: 'full name',
      idNumber: '746298',
      dateOfBirth: '12/08/91',
      expirationDate: '17/12/49',
      photo: ''
    },
    {
      externalid: '7bdde45f-8840-4fc2-b43e-471db1e1748f',
      contactName: 'contact name',
      fullname: 'full name',
      idNumber: '752844',
      dateOfBirth: '12/08/91',
      expirationDate: '17/12/49',
      photo: ''
    },
    {
      externalid: 'd3795d9e-32cf-48f6-8a4b-14517049c172',
      contactName: 'contact name',
      fullname: 'full name',
      idNumber: '759390',
      dateOfBirth: '12/08/91',
      expirationDate: '17/12/49',
      photo: ''
    },
    {
      externalid: '1527aad2-c4af-4ca8-8c62-4b6e82f77609',
      contactName: 'contact name',
      fullname: 'full name',
      idNumber: '765936',
      dateOfBirth: '12/08/91',
      expirationDate: '17/12/49',
      photo: ''
    },
    {
      externalid: '682373fc-f13b-4e39-9b3e-e46c5c234499',
      contactName: 'contact name',
      fullname: 'full name',
      idNumber: '772482',
      dateOfBirth: '12/08/91',
      expirationDate: '17/12/49',
      photo: ''
    },
    {
      externalid: 'a96953c3-5ddc-4c61-bdb2-f4bd87d1d58e',
      contactName: 'contact name',
      fullname: 'full name',
      idNumber: '779028',
      dateOfBirth: '12/08/91',
      expirationDate: '17/12/49',
      photo: ''
    }
  ]
};

export const extraHeaders = {
  inputAoA: [
    [
      'External ID',
      'Contact Name',
      'ID Number',
      'Full Name',
      'Date Of Birth',
      'Expiration Date',
      'Photo',
      'extraHeader1',
      '',
      'extraHeader3'
    ],
    [
      '501619c5-270a-4e80-9e70-f0cf9bcef5e5',
      'contact name',
      '654654',
      'full name',
      '12/08/91',
      '17/12/49',
      '',
      '',
      '',
      ''
    ],
    [
      '14c17a25-30a9-4e3d-a63d-83ebd8a2dd59',
      'contact name',
      '661200',
      'full name',
      '12/08/91',
      '17/12/49',
      '',
      '',
      '',
      ''
    ],
    [
      '614e9d7a-71c8-4a31-a1d2-8c1928c0541d',
      'contact name',
      '667746',
      'full name',
      '12/08/91',
      '17/12/49',
      '',
      '',
      '',
      ''
    ]
  ],
  expectedErrors: [
    [
      { error: 'excessHeader', row: { index: -1 }, col: { index: 7, name: 'extraHeader1' } },
      { error: 'excessHeader', row: { index: -1 }, col: { index: 8, name: '' } },
      { error: 'excessHeader', row: { index: -1 }, col: { index: 9, name: 'extraHeader3' } }
    ]
  ],
  expectedParse: [
    {
      externalid: '501619c5-270a-4e80-9e70-f0cf9bcef5e5',
      contactName: 'contact name',
      fullname: 'full name',
      idNumber: '654654',
      dateOfBirth: '12/08/91',
      expirationDate: '17/12/49',
      photo: ''
    },
    {
      externalid: '14c17a25-30a9-4e3d-a63d-83ebd8a2dd59',
      contactName: 'contact name',
      fullname: 'full name',
      idNumber: '661200',
      dateOfBirth: '12/08/91',
      expirationDate: '17/12/49',
      photo: ''
    },
    {
      externalid: '614e9d7a-71c8-4a31-a1d2-8c1928c0541d',
      contactName: 'contact name',
      fullname: 'full name',
      idNumber: '667746',
      dateOfBirth: '12/08/91',
      expirationDate: '17/12/49',
      photo: ''
    }
  ]
};

export const invalidHeadersOrder = {
  inputAoA: [
    [
      'External ID',
      'Full Name',
      'Contact Name',
      'Photo',
      'Date Of Birth',
      'ID Number',
      'Expiration Date'
    ],
    [
      '501619c5-270a-4e80-9e70-f0cf9bcef5e5',
      'full name',
      'contact name',
      '',
      '12/08/91',
      '654654',
      '17/12/49'
    ]
  ],
  expectedErrors: [
    [
      {
        error: 'invalidHeaderPosition',
        row: { index: -1 },
        col: { index: 1, expectedIndex: 3, name: 'Full Name' }
      },
      {
        error: 'invalidHeaderPosition',
        row: { index: -1 },
        col: { index: 2, expectedIndex: 1, name: 'Contact Name' }
      },
      {
        error: 'invalidHeaderPosition',
        row: { index: -1 },
        col: { index: 3, expectedIndex: 6, name: 'Photo' }
      },
      {
        error: 'invalidHeaderPosition',
        row: { index: -1 },
        col: { index: 5, expectedIndex: 2, name: 'ID Number' }
      },
      {
        error: 'invalidHeaderPosition',
        row: { index: -1 },
        col: { index: 6, expectedIndex: 5, name: 'Expiration Date' }
      }
    ]
  ],
  expectedParse: [
    {
      externalid: '501619c5-270a-4e80-9e70-f0cf9bcef5e5',
      contactName: 'contact name',
      fullname: 'full name',
      idNumber: '654654',
      dateOfBirth: '12/08/91',
      expirationDate: '17/12/49',
      photo: ''
    }
  ]
};

export const invalidCredentialsData = {
  inputAoA: [
    [
      'External ID',
      'Contact Name',
      'ID Number',
      'Full Name',
      'Date Of Birth',
      'Expiration Date',
      'Photo'
    ],
    ['501619c5-270a-4e80-9e70-f0cf9bcef5e5', 'contact name', '', 'full name', '12/08/91', '', ''],
    ['', 'contact name', '661200', '', '', '17/12/49', ''],
    [
      '614e9d7a-71c8-4a31-a1d2-8c1928c0541d',
      'contact name',
      '667746',
      'full name',
      '12/08/91',
      '17/12/49',
      ''
    ],
    ['incorrectExternalId', 'contact name', '779028', 'full name', '12/08/91', '17/12/49', '']
  ],
  expectedErrors: [
    [],
    [
      {
        error: 'required',
        row: { index: 0 },
        col: { index: 2, name: 'ID Number' }
      },
      {
        error: 'required',
        row: { index: 0 },
        col: { index: 5, name: 'Expiration Date' }
      }
    ],
    [
      {
        error: 'required',
        row: { index: 1 },
        col: { index: 0, name: 'External ID' }
      },
      {
        error: 'valueDoesNotMatch',
        row: { index: 1 },
        col: { index: 1, name: 'Contact Name' }
      },
      {
        error: 'required',
        row: { index: 1 },
        col: { index: 3, name: 'Full Name' }
      },
      {
        error: 'required',
        row: { index: 1 },
        col: { index: 4, name: 'Date Of Birth' }
      }
    ],
    [],
    [
      {
        error: 'unexpectedExternalID',
        row: { index: 3 },
        col: { index: 0, name: 'External ID' }
      },
      {
        error: 'valueDoesNotMatch',
        row: { index: 3 },
        col: { index: 1, name: 'Contact Name' }
      }
    ]
  ],
  expectedParse: [
    {
      externalid: '501619c5-270a-4e80-9e70-f0cf9bcef5e5',
      contactName: 'contact name',
      fullname: 'full name',
      idNumber: '',
      dateOfBirth: '12/08/91',
      expirationDate: '',
      photo: ''
    },
    {
      externalid: '',
      contactName: 'contact name',
      idNumber: '661200',
      fullname: '',
      dateOfBirth: '',
      expirationDate: '17/12/49',
      photo: ''
    },
    {
      externalid: '614e9d7a-71c8-4a31-a1d2-8c1928c0541d',
      contactName: 'contact name',
      idNumber: '667746',
      fullname: 'full name',
      dateOfBirth: '12/08/91',
      expirationDate: '17/12/49',
      photo: ''
    },
    {
      contactName: 'contact name',
      dateOfBirth: '12/08/91',
      expirationDate: '17/12/49',
      externalid: 'incorrectExternalId',
      fullname: 'full name',
      idNumber: '779028',
      photo: ''
    }
  ]
};

export const emptyRows = {
  inputAoA: [
    [
      'External ID',
      'Contact Name',
      'ID Number',
      'Full Name',
      'Date Of Birth',
      'Expiration Date',
      'Photo'
    ],
    [
      '501619c5-270a-4e80-9e70-f0cf9bcef5e5',
      'contact name',
      '654654',
      'full name',
      '12/08/91',
      '17/12/49',
      ''
    ],
    [
      '14c17a25-30a9-4e3d-a63d-83ebd8a2dd59',
      'contact name',
      '661200',
      'full name',
      '12/08/91',
      '17/12/49',
      ''
    ],
    ['', '', '', '', '', '', ''],
    [
      '614e9d7a-71c8-4a31-a1d2-8c1928c0541d',
      'contact name',
      '667746',
      'full name',
      '12/08/91',
      '17/12/49',
      ''
    ],
    ['', '', '', '', '', '', ''],
    []
  ],
  expectedErrors: [
    [],
    [],
    [],
    [
      {
        error: 'emptyRow',
        row: { index: 2 },
        col: { index: 0 }
      }
    ],
    []
  ],
  expectedParse: [
    {
      externalid: '501619c5-270a-4e80-9e70-f0cf9bcef5e5',
      contactName: 'contact name',
      fullname: 'full name',
      idNumber: '654654',
      dateOfBirth: '12/08/91',
      expirationDate: '17/12/49',
      photo: ''
    },
    {
      externalid: '14c17a25-30a9-4e3d-a63d-83ebd8a2dd59',
      contactName: 'contact name',
      fullname: 'full name',
      idNumber: '661200',
      dateOfBirth: '12/08/91',
      expirationDate: '17/12/49',
      photo: ''
    },
    {
      externalid: '',
      contactName: '',
      fullname: '',
      idNumber: '',
      dateOfBirth: '',
      expirationDate: '',
      photo: ''
    },
    {
      externalid: '614e9d7a-71c8-4a31-a1d2-8c1928c0541d',
      contactName: 'contact name',
      fullname: 'full name',
      idNumber: '667746',
      dateOfBirth: '12/08/91',
      expirationDate: '17/12/49',
      photo: ''
    },
    {
      externalid: '',
      contactName: '',
      fullname: '',
      idNumber: '',
      dateOfBirth: '',
      expirationDate: '',
      photo: ''
    },
    {
      externalid: undefined,
      contactName: undefined,
      fullname: undefined,
      idNumber: undefined,
      dateOfBirth: undefined,
      expirationDate: undefined,
      photo: undefined
    }
  ]
};

export const emptyData = {
  inputAoA: [
    [
      'External ID',
      'Contact Name',
      'ID Number',
      'Full Name',
      'Date Of Birth',
      'Expiration Date',
      'Photo'
    ]
  ],
  expectedErrors: [
    [
      {
        error: 'emptyFile',
        row: { index: 0 },
        col: { index: 0 }
      }
    ]
  ],
  expectedParse: []
};

export const emptyFile = {
  inputAoA: [[]],
  expectedErrors: [
    [
      {
        error: 'emptyFile',
        row: { index: 0 },
        col: { index: 0 }
      }
    ]
  ],
  expectedParse: []
};

export const invalidDates = {
  inputAoA: [
    [
      'External ID',
      'Contact Name',
      'ID Number',
      'Full Name',
      'Date Of Birth',
      'Expiration Date',
      'Photo'
    ],
    [
      '501619c5-270a-4e80-9e70-f0cf9bcef5e5',
      'contact name',
      '654654',
      'full name',
      '12/08/91',
      '17/12/49',
      ''
    ],
    [
      '14c17a25-30a9-4e3d-a63d-83ebd8a2dd59',
      'contact name',
      '661200',
      'full name',
      '12/08/1991',
      '17/12/2049',
      ''
    ],
    [
      '614e9d7a-71c8-4a31-a1d2-8c1928c0541d',
      'contact name',
      '667746',
      'full name',
      '656565',
      '565656',
      ''
    ],
    [
      'd920875f-ddec-4c0b-ab1c-5c2b9c8ee415',
      'contact name',
      '674292',
      'full name',
      '12/08/2091',
      '17/12/1949',
      ''
    ]
  ],
  expectedErrors: [
    [],
    [],
    [],
    [
      {
        error: 'dateFormat',
        row: { index: 2 },
        col: { index: 4, name: 'Date Of Birth' }
      },
      {
        error: 'dateFormat',
        row: { index: 2 },
        col: { index: 5, name: 'Expiration Date' }
      }
    ],
    [
      {
        error: 'notAPastDate',
        row: { index: 3 },
        col: { index: 4, name: 'Date Of Birth' }
      },
      {
        error: 'notAFutureDate',
        row: { index: 3 },
        col: { index: 5, name: 'Expiration Date' }
      }
    ]
  ],
  expectedParse: [
    {
      externalid: '501619c5-270a-4e80-9e70-f0cf9bcef5e5',
      contactName: 'contact name',
      fullname: 'full name',
      idNumber: '654654',
      dateOfBirth: '12/08/91',
      expirationDate: '17/12/49',
      photo: ''
    },
    {
      externalid: '14c17a25-30a9-4e3d-a63d-83ebd8a2dd59',
      contactName: 'contact name',
      fullname: 'full name',
      idNumber: '661200',
      dateOfBirth: '12/08/1991',
      expirationDate: '17/12/2049',
      photo: ''
    },
    {
      externalid: '614e9d7a-71c8-4a31-a1d2-8c1928c0541d',
      contactName: 'contact name',
      fullname: 'full name',
      idNumber: '667746',
      dateOfBirth: '656565',
      expirationDate: '565656',
      photo: ''
    },
    {
      externalid: 'd920875f-ddec-4c0b-ab1c-5c2b9c8ee415',
      contactName: 'contact name',
      fullname: 'full name',
      idNumber: '674292',
      dateOfBirth: '12/08/2091',
      expirationDate: '17/12/1949',
      photo: ''
    }
  ]
};
