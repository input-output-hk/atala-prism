...
/**
  ...
  * <a href="https://github.com/input-output-hk/atala/blob/develop/credentials-verification/docs/cardano/use-cardano.md">here</a>.
  */
class CardanoFeeEstimator(walletId: WalletId, paymentAddress: Address, cardanoWalletApiClient: CardanoWalletApiClient) {
  ...
  def estimate(issuers: List[Issuer]): TotalEstimation = {
    ...
    issuers.foreach { issuer =>
      ...
      val masterKeyOperation = addMasterKeyOperation(did, masterKey.publicKey)
      createDidAtalaObjects += createAtalaObject(
        signOperation(masterKeyOperation, masterKey.privateKey),
        signOperation(addIssuingKeyOperation(did, issuingKey.publicKey, masterKeyOperation), masterKey.privateKey)
      )
      ...
      issuer.credentialsToIssue.foreach { credentialsToIssue =>
        ...
        for (batchId <- 0 until batches) {
          val merkleRoot = MerkleRoot(SHA256Digest.compute(s"Issuer ${issuer.id}, batch $batchId".getBytes))
          issueCredentialBatchAtalaObjects += createAtalaObject(
            signOperation(issueCredentialBatchOperation(merkleRoot, did), issuingKey.privateKey)
          )
        }
      }
    }
  ...
  private def createDID(id: String): DIDFormat.Canonical = {
    DIDFormat.Canonical(SHA256Digest.compute(id.getBytes).hexValue)
  }

  private def addMasterKeyOperation(did: DIDFormat.Canonical, publicKey: ECPublicKey): AtalaOperation = {
  ...
  private def addIssuingKeyOperation(
      did: DIDFormat.Canonical,
      publicKey: ECPublicKey,
      lastOperation: AtalaOperation
  ): AtalaOperation = {
    val createDIDOp = node_models.UpdateDIDOperation(
      previousOperationHash = ByteString.copyFrom(SHA256Digest.compute(lastOperation.toByteArray).value.toArray),
      id = did.suffix,
      actions = List(
  ...
  private def issueCredentialBatchOperation(merkleRoot: MerkleRoot, issuerDid: DIDFormat.Canonical): AtalaOperation = {
