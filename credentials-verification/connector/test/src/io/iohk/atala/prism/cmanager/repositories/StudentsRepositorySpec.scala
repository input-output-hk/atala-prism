...
class StudentsRepositorySpec extends CManagerRepositorySpec {
  ...
  "create" should {
    "create a new student and assign it to a group" in {
      ...
      val request = CreateStudent(issuerId, "uid", "Dusty Here", "d.here@iohk.io", LocalDate.now())
      ...
      val studentsInGroupList = repository.getBy(issuerId, 10, None, Some(group.name)).value.futureValue.right.value
    ...
    "create a new student and assign it to no group" in {
      ...
      val maybeStudent = repository.find(issuerId, student.id).value.futureValue.right.value.value
    ...
    "fail to create a new student when assigned to a group that does not exist" in {
      ...
      val studentsList = repository.getBy(issuerId, 1, None, None).value.futureValue.right.value
  ...
  "getBy" should {
    "return the first students" in {
      ...
      val credA = createStudent(issuerId, "A")
      val credB = createStudent(issuerId, "B")
      createStudent(issuerId, "C")
    ...
    "return the first students matching a group" in {
      val issuerId = createIssuer("Issuer X")
      val groupA = createIssuerGroup(issuerId, IssuerGroup.Name("Group 1"))
      val groupB = createIssuerGroup(issuerId, IssuerGroup.Name("Group 2"))

      val credA = createStudent(issuerId, "A", Some(groupA))
      createStudent(issuerId, "B", Some(groupB))
      val credC = createStudent(issuerId, "C", Some(groupA))

      val result = repository.getBy(issuerId, 2, None, Some(groupA.name)).value.futureValue.right.value
      result.toSet must be(Set(credA, credC))
    }

    "paginate by the last seen student" in {
      val issuerId = createIssuer("Issuer X")
      createStudent(issuerId, "A")
      createStudent(issuerId, "B")
      val credC = createStudent(issuerId, "C")
      createStudent(issuerId, "D")

      val first = repository.getBy(issuerId, 2, None, None).value.futureValue.right.value
      val result = repository.getBy(issuerId, 1, first.lastOption.map(_.id), None).value.futureValue.right.value
    ...
    "paginate by the last seen student matching by group" in {
      ...
      createStudent(issuerId, "A", Some(groupA))
      createStudent(issuerId, "B", Some(groupA))
      createStudent(issuerId, "C", Some(groupB))
      val credD = createStudent(issuerId, "D", Some(groupA))

      val first = repository.getBy(issuerId, 2, None, Some(groupA.name)).value.futureValue.right.value
      val result = repository
        .getBy(issuerId, 1, first.lastOption.map(_.id), Some(groupA.name))
        .value
        .futureValue
        .right
        .value
  ...
  "generateToken" should {
    "update the student to set the status and token" in {
      val issuerId = createIssuer("tokenizer")
      val student = createStudent(issuerId, "token")
      val result = repository.generateToken(issuerId, student.id).value.futureValue
      val token = result.right.value

      val newStudent = repository.find(issuerId, student.id).value.futureValue.right.value.value
  ...
  private def createStudent(issuer: Institution.Id, tag: String, groupMaybe: Option[IssuerGroup] = None): Student = {
