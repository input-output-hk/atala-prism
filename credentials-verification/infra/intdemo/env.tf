...
resource aws_lb_listener landing-lb-listener {
  load_balancer_arn = aws_lb.ecs-net-lb.arn
  protocol          = "TCP"
  ...
  default_action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.landing-target-group.arn
  }
}

resource aws_lb_target_group landing-target-group {
  name     = "landing-tg-${var.env_name_short}"
  protocol = "TCP"
  port     = var.landing_port
  vpc_id   = var.credentials-vpc-id
...
resource aws_autoscaling_group ec2-autoscaling-group {
  ...
  target_group_arns = [
    aws_lb_target_group.landing-target-group.arn,
...
data "template_file" "intdemo-task-template" {
  template = file("intdemo.task.json")
  vars = {
    ...
    landing-docker-image   = var.landing_docker_image
    envoy-docker-image     = var.envoy_docker_image

    awslogs-region = var.aws_region
    awslogs-group  = aws_cloudwatch_log_group.intdemo-log-group.name

    react-app-grpc-client = "http://intdemo-${var.env_name_short}.cef.iohkdev.io:${var.envoy_port}"
  }
}

resource aws_ecs_task_definition intdemo-task-definition {
  family                = "intdemo-task-def-${var.env_name_short}"
  container_definitions = data.template_file.intdemo-task-template.rendered
  network_mode          = "bridge"
  tags = {
...
resource aws_ecs_service intdemo-service {
  name            = "intdemo-service-${var.env_name_short}"
  cluster         = aws_ecs_cluster.credentials-cluster.id
  task_definition = aws_ecs_task_definition.intdemo-task-definition.arn
  desired_count   = 2
...
resource aws_route53_record intdemo-dns-entry {
...
resource aws_cloudwatch_log_group intdemo-log-group {
...
output command-to-test-connector {
  value = "grpcurl -import-path ../../connector/protobuf/connector -proto protos.proto -rpc-header 'userId: c8834532-eade-11e9-a88d-d8f2ca059830' -plaintext intdemo-${var.env_name_short}.cef.iohkdev.io:${var.connector_port} io.iohk.cvp.connector.ConnectorService/GenerateConnectionToken"
}

output command-to-test-envoy-proxy {
  value = "curl -i -XOPTIONS -H'Host: intdemo-${var.env_name_short}.cef.iohkdev.io:8080' -H'Accept: */*' -H'Accept-Language: en-GB,en;q=0.5' -H'Accept-Encoding: gzip, deflate' -H'Access-Control-Request-Method: POST' -H'Access-Control-Request-Headers: content-type,userid,x-grpc-web,x-user-agent' -H'Referer: http://localhost:3000/connections' -H'Origin: http://localhost:3000' 'http://intdemo-${var.env_name_short}.cef.iohkdev.io:${var.envoy_port}/io.iohk.cvp.connector.ConnectorService/GenerateConnectionToken'"
}

output "command-to-test-intdemo-web-app" {
  value = "curl -i 'http://intdemo-${var.env_name_short}.cef.iohkdev.io'"
}
