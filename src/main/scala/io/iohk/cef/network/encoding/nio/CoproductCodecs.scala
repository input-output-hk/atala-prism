...
trait CoproductCodecs {
  ...
  implicit def optionEncoder[T: NioEncoder: TypeTag]: NioEncoder[Option[T]] = new NioEncoder[Option[T]] {
    override def encode(o: Option[T]): ByteBuffer =
      messageLengthEncoder(typeCodeEncoder(optionEncoderImpl[T])).encode(o)
  }

  private def optionEncoderImpl[T](implicit enc: NioEncoder[T], t: TypeTag[T]): NioEncoder[Option[T]] =
    new NioEncoder[Option[T]] {
      override def encode(o: Option[T]): ByteBuffer = o.fold(ByteBuffer.allocate(0))(t => enc.encode(t))
    }

  implicit def optionDecoder[T: NioDecoder: TypeTag]: NioDecoder[Option[T]] = new NioDecoder[Option[T]] {
    override def decode(b: ByteBuffer): Option[Option[T]] =
      messageLengthDecoder(typeCodeDecoder(optionDecoderImpl[T])).decode(b)
  }

  private def optionDecoderImpl[T](implicit dec: NioDecoder[T], t: TypeTag[T]): NioDecoder[Option[T]] =
  ...
  implicit def eitherEncoder[L: NioEncoder: TypeTag, R: NioEncoder: TypeTag]: NioEncoder[Either[L, R]] =
  ...
  private def eitherEncoderImpl[L, R](
      ...
      rEnc: NioEncoder[R],
      lt: TypeTag[L],
      rh: TypeTag[R]): NioEncoder[Either[L, R]] = new NioEncoder[Either[L, R]] {
    override def encode(e: Either[L, R]): ByteBuffer = {
      e.fold(l => leftEncoder[L, R].encode(Left(l)), r => rightEncoder[L, R].encode(Right(r)))
    }
  }

  private def leftEncoder[L, R](implicit lEnc: NioEncoder[L], lt: TypeTag[L], rt: TypeTag[R]): NioEncoder[Left[L, R]] =
  ...
  private def rightEncoder[L, R](
      ...
      lt: TypeTag[L],
      rt: TypeTag[R]): NioEncoder[Right[L, R]] =
  ...
  implicit def eitherDecoder[L: NioDecoder: TypeTag, R: NioDecoder: TypeTag]: NioDecoder[Either[L, R]] =
  ...
  private def eitherDecoderImpl[L, R](
      ...
      rDec: NioDecoder[R],
      lt: TypeTag[L],
      rt: TypeTag[R]): NioDecoder[Either[L, R]] = new NioDecoder[Either[L, R]] {
    override def decode(b: ByteBuffer): Option[Either[L, R]] =
  ...
  private def leftDecoder[L, R](implicit lDec: NioDecoder[L], lt: TypeTag[L], rt: TypeTag[R]): NioDecoder[Left[L, R]] =
  ...
  private def rightDecoder[L, R](
      ...
      lt: TypeTag[L],
      rt: TypeTag[R]): NioDecoder[Right[L, R]] =
