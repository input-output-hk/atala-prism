...
trait LedgerStateStorageDaoDbTest extends fixture.FlatSpec with AutoRollback with MustMatchers with MockitoSugar {
  ...
  it should "retrieve an slice" in { implicit session =>
    ...
    when(byteStringSerializable.encode(publicKeys)).thenReturn(serializedKeys)
    when(byteStringSerializable.decode(serializedKeys)).thenReturn(Some(publicKeys))
    ...
    when(byteStringSerializable.encode(otherPublicKeys)).thenReturn(otherSerializedKeys)
    when(byteStringSerializable.decode(otherSerializedKeys)).thenReturn(Some(otherPublicKeys))
  ...
  it should "update a state" in { implicit session =>
    ...
    when(byteStringSerializable.encode(publicKeys)).thenReturn(serializedKeys)
    when(byteStringSerializable.decode(serializedKeys)).thenReturn(Some(publicKeys))
    when(byteStringSerializable.encode(publicKey1Set)).thenReturn(serializedPublicKey1Set)
    when(byteStringSerializable.decode(serializedPublicKey1Set)).thenReturn(Some(publicKey1Set))
    val dao = new LedgerStateStorageDao[Set[ByteString]]()
    implicit val bsSerializable = byteStringSerializable
